...
## 2026-07-03 · P3.5.173 — XlsxImportModal: 从 xlsx 批量抽体系 + 部门树建 wiki

### 鸿波 catch

上传"福富组织架构20260423.xlsx" (5 sheets: 3 体系 + 100+ 部门), 期望 catfish 自动
抽体系 concept + 部门 entity 建关系树, 严格实际只手动创建了单个"组织架构" concept
(body 是全 xlsx 表格明文). 鸿波追问"为什么读入会出错".

### Phase A/B audit — 严格代码事实 (root cause)

严格 audit catfish 现有能力:
- **catfish-memory `sync_turn`** (5/20): 只做 buffer 累积 + summary 触发 +
  strategic_docs 注入, 严格**不抽 wiki**
- **P16 attachment ingest** (6/5): 只把 xlsx 全文存 `~/.catfish/wiki/raw/sources/`,
  严格**不生成 concept/entity**
- **WikiCreateModal** (P3.5.109): 员工**手动**一次一个填 title/kind/related/body
- **P3.5.172 "🔗 扫描关联"**: 只**关联现有节点**, 严格**不新建 entity**

**严格 root cause = 功能缺失, 不是 bug**. 员工手动创建"组织架构" 是**手动填**了
整个 body 表格, catfish 严格没帮做 batch 抽取.

### Phase A audit (6 层复用/依赖)

严格 audit 拿 6 层代码事实:
- ❌ **前端 xlsx lib**: 无 SheetJS, 需 `npm i xlsx` (~500KB)
- ❌ **Rust xlsx lib**: `rust_xlsxwriter 0.79` **只 write**, 无 calamine (read)
- ❌ **Tauri dialog plugin**: 无引, ChatInput 严格用 native `<input type="file">`
  (line 423-429) 就够 — **复用此模式**
- ✅ **WikiTree "+新建" 位置** (line 275-282): 可加 "📊" 按钮
- ✅ **wikiCreateEntityOrConcept**: tauri async, for-loop await 严格 sequential
  保 fs 安全
- ✅ **现有 modal 模式**: SetupModal/LearnModal/WikiCreate/WikiLinkSuggest 全单
  页. XlsxImport 严格需 stage machine (3 stage)

### 设计决策

- **xlsx lib**: 前端 SheetJS (`npm i xlsx`), 数据主权 (员工 mac local parse),
  无 Cargo 编译
- **file picker**: 复用 ChatInput 模式 `<input type="file" accept=".xlsx,.xls,.xlsm">`
- **UX 3 stage**: 选文件 → parse+preview tree (员工 checkbox) → batch create+progress
- **wikilink 关联树**: 组织架构 → 3 concept → 一级部 → 二级部, 严格自动挂 related

### Phase B 实现 (2 新文件, ~1150 行)

**`lib/xlsxImport.ts`** (~140 行) — 严格 xlsx heuristic parser:
- `parseOrgXlsx(buf) → XlsxSheetTree[]`
- 严格 heuristic: 找 header 行含 "一级部" → col idx → next rows col_idx=一级部,
  col_idx+1=二级部
- 严格 forward-fill 一级部 (merged cell blank → 取上一 non-blank)
- 严格 skip: sheet name 含"总图" / "发文" / "调整" / "说明" / "Sheet1" 严格 skip
- `countXlsxTree` + `countSelectedTree` helper

**`tabs/Wiki/XlsxImportModal.tsx`** (~700 行) — 3 stage state machine:
- **Stage 1 pick**: native `<input type="file">` file picker + 说明 hint
- **Stage 2 preview**: PreviewTree sub-component 严格 3 层 tree checkbox
  (concept / 一级部 / 二级部) + cascading toggle + rename rootTitle + counter
- **Stage 3 importing**: ImportProgressView 严格 progress bar + current name +
  success/fail counter
- **Stage 4 done/error**: 严格完成 toast + 失败详情 details
- 严格错误 skip 继续 (不阻塞 batch), "已存在" 严格算成功 skip
- 严格 batch 顺序: rootTitle (若不存在) → 3 concept → 一级部 → 二级部
- 严格 related 严格自动挂 wikilink 建树

### Phase C — WikiTree 加 button

WikiTree.tsx 严格改动:
- Import `XlsxImportModal`
- 加 `[xlsxImportOpen, setXlsxImportOpen]` state (内嵌 useState, 无独立 store)
- 严格加 📊 button 在 "+新建" 右侧, title="从 xlsx 批量导入组织架构"
- 严格 Modal render 在末尾, `onImported` → `loadFiles()`

### 员工体验对比

**老 (P3.5.172)**:
1. 上传 xlsx 给 catfish → **无 batch 抽取**
2. 员工手动 "+新建" 一次一个 (100+ entity × 5 field = 500+ 次点击)
3. 严格 5+ 小时

**新 (P3.5.173)**:
1. WikiTree 点 📊 → 弹 XlsxImportModal
2. 选 xlsx 文件 (3 秒)
3. 前端 SheetJS 严格 parse → 显 tree preview (3 concept + 100+ 一级/二级) (2 秒)
4. 员工 checkbox 严格确认 (默认全选, 员工可 uncheck 不需要的) (30 秒)
5. 点"开始导入 →" → progress bar 严格 batch create ~100 项 (~30 秒)
6. 完成 toast 显 "成功 X 项 / 失败 Y 项" → WikiGraph 自动更新拉满树

**严格 5+ 小时 → 1 分钟**.

### 数据主权 & 军规

- ✅ **员工主权**: 员工 checkbox 每层确认哪些建, 不自动
- ✅ **数据零出端**: SheetJS 严格前端 parse (员工 mac local), xlsx 严格不走中央
- ✅ **UX 一致**: 复用 RecMode ModalShell + T theme (跟 SetupModal/LearnModal/
  WikiLinkSuggest 同风)
- ✅ **错误 fault-tolerant**: skip 已存在 (不算错), 前 10 条错误详情记录
- ✅ **复用现有 API**: `wikiCreateEntityOrConcept` 严格已有, 无新 tauri command
...
装完严格 `npm run tauri dev` 即可用. tsc 严格 sandbox 报 `Cannot find module 'xlsx'`
是 lib 未装, 员工装完 pass.

### 军规自查

- ✅ 严格 Phase A audit 6 层代码事实 (前端 lib / Rust lib / dialog plugin /
  button 位置 / 批量 API / modal 模式), 严格不瞎猜
- ✅ 严格 Phase B 复用 profile.ts 8999 直连模式 → SheetJS 前端 parse (数据主权)
- ✅ 严格 Phase C 复用 ModalShell + T theme, 复用 native `<input type="file">`
  ChatInput 模式
- ✅ 承认 P3.5.172 "🔗 扫描关联" 只关联现有, 严格没考虑批量抽取新体系场景 —
  P3.5.173 严格补
- ✅ 承认 sandbox 无 network 装 `npm i xlsx`, tsc 严格 sandbox 报 lib 未找到, 严格
  说明员工 mac 装完 pass. 不假装 tsc 全绿

### 遗留

- **员工 mac 装 xlsx lib** — `npm i xlsx` 一次性, 后续可用
- **rename 未实现** — 员工 preview 时若想改 title, 严格得导入完再改 (backlog)
- **subtype 严格默认**: concept=system, entity=org. 严格员工场景可能需其他
  subtype (e.g. cert), 严格未来加 modal 侧 subtype 选 (backlog)
- **进度可 abort** — 严格现在 batch 中不可 cancel, 严格未来加 abort button (backlog)
