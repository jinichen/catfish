// 稻生万物智能制造建设方案 · v29 · 24 页
//
// ═══ v29 vs v28 (7/28 鸿波第十七轮) ═══
//
// 鸿波: "客户提了计算化学的概念…… 如果不加，是不是可以在研的部分提一下？"
// 采纳: 不单独成页, 在 P7 研发 Agent 里轻点三处。
//
// ═══ 为什么不单独成页 ═══
//
// 计算化学需要专门的人（量子化学 / 分子模拟背景）与专门软件, 不是平台团队
// 顺手能做的事。本方案花了十几轮把边界立住（不碰设备、视觉检测交厂商、
// 不进控制回路）—— 为迎合客户把它写成我方能力, 等于自破规矩,
// 且是承诺一个不具备的能力。
//
// ═══ 但确实该提, 因为有一句真话只有我们能说 ═══
//
//   计算化学最大的实际问题不是算不出来, 是**算完了留不下**。
//   典型情况: 请高校做个课题, 算出一批结果, 报告交了,
//   数据散在某个学生的电脑里; 下次遇到类似问题重新算一遍。
//
// 我方能做的是把计算结果作为**一类数据源纳入知识层**, 与实验、中试、
// 产线数据关联: 计算预测的界面结合能 ↔ 实际配方的力学性能 ↔ 产线真实表现,
// 形成「计算 → 实验 → 中试 → 量产」的闭环验证, 沉淀为可复用资产而非
// 一次性报告。这与 P6 上下文工程、P11 知识层同一条逻辑,
// 也与视觉质检那页同构（厂商做检测, 我方做归因）。
//
// ═══ 对本项目最有价值的应用点（供答客户问, 未写进 PPT）═══
//
//   界面相容性 —— 天然纤维复合材料的头号瓶颈: 稻壳纤维亲水极性、
//   聚合物基体疏水非极性, 结合不牢则力学性能上不去。计算可在合成与试制前
//   筛选偶联剂 / 相容剂候选（溶解度参数、界面结合能模拟）, 缩小试错范围。
...
//
// ═══ v29 具体改动（仅 P7, 页数不变）═══
//   ① 输入列表 增列"外部计算化学 / 分子模拟结果（如有合作方产出）"
//   ② 「候选方向建议」任务 补: 计算筛选结果可一并纳入比对
//   ③ 页脚加一行边界注: 谁来算 / 我方承担什么
//
// ═══ 延续的结构性决定 ═══
...
//   - **不把外部专业能力写成我方能力** —— 计算化学明确标注由外部机构承担
//   - 不出现申报 / 政策 / 补贴 / 固投 / 管委会
...
p.writeFile({ fileName: "daosheng-v29-smart-mfg.pptx" })
  .then(() => console.log("✓ daosheng-v29-smart-mfg.pptx  ·  24 页"));
