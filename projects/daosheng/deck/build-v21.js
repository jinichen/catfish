// 稻生万物研产销一体化产业园 · AI Native 数字化方案 · v21 · 22 页
//
// ═══ v21 vs v20 (7/28 鸿波第九轮 · 范式重构) ═══
//
// 鸿波: "我觉得这个结构不对，应该按照 AI native 的方式来设计，而不是现在这种拼凑"
//
// 这个批评指到根子上了。v20 的骨架是**传统企业信息化**:
//   MES / QMS / 追溯 / 认证档案 / 订单系统 各建一套, AI 塞在 P14-P15 当附加功能。
//   先搭一套 2010 年代的系统, 再在上面撒一层 AI —— 确实是拼凑。
//
// ═══ 为什么这个项目该是 AI Native（P2 的论证）═══
//
// 不是技术时髦, 是这个园区的商业模式决定的:
//
//   1. 这家企业的核心资产全是**隐性知识** —— 配方经验、老师傅调参手感、
//      国际认证踩坑经验、品牌客户对接经验, 全在人脑子里
//   2. 传统信息化管的是流程与单据, 它要求**人先把知识填成表单**再管理
//   3. 但人不愿填也填不完整 —— v18 的 P8 那条"研发不愿改记录习惯"就是这么来的
//   4. 而园区模式是"链主赋能上下游", **经验不能规模化输出, 园区就不成立**
//   5. AI Native 把这件事反过来: 人照常工作, 系统从过程中提取知识;
//      下游用自然语言调用, 不用学系统
//   6. 补充: 入驻企业多为中小企业, 没有 IT 团队 —— 给账号也用不起来, 给 Agent 才用得动
//
// ═══ 结构重构 ═══
//
//   旧(v20)  应用层(MES/QMS/CRM…) → 数据平台 → 采集     · AI 是附加模块
//   新(v21)  交互 → Agent 编排 → 能力池+知识记忆 → 数据语义层 → 接入
//                                  · 传统系统降级为 Agent 可调用的工具(P12 专讲)
//
//   研产销不再是"三组功能模块", 而是**四个 Agent**:
//     P7  研发 Agent      从实验过程中提取知识, 而非要求研发填表
//     P8  生产 Agent      持续观察→归因→建议, 而非人查报表
//     P9  合规与客户 Agent 理解客户来函与标准文件, 自动组包与识别缺口
//     P10 园区 Agent      ★ 链主赋能规模化的载体 —— 把能力而非账号开放给下游
//
// ═══ 三个必须防住的坑 ═══
//
//   ① 不能变成概念秀 —— 每个 Agent 都写清具体任务、人机分工、落地前提
//   ② 不能让客户以为"不用建 MES 了" —— P12 专门讲传统系统的新位置:
//      不是不建, 是从"人操作的系统"变成"Agent 调用的工具与数据源"
//   ③ 申报评审专家不讲 AI Native —— P18 做口径翻译, 把架构映射到技改/研发政策语言
//
// 能力池的内容基于对既有能力的真实盘点(工具链 / 技能凝固闭环 / 外部系统接入 /
// 多模型路由 / 定时触发 / 视觉理解), 不按宣传口径写。
//
// ═══ 延续的结构性决定 ═══
//   ① 交付边界在"数据接入层"切一刀, 设备侧与视觉检测硬件交集成商, 现场零部署 (P13)
//   ② 接口规范仍由本方定义 —— 边界是"不实施"不是"不介入"
//   ③ 商业逻辑压成 P2 背景, 完整版见 v15/v16
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - 不写人月工期
//   - 指标基线 [待实测] / 目标 [待定]
//   - **AI 效果不承诺数字** —— 准确率/提升幅度一律不写
//   - **能力只写真实能做的** —— 不写"全自动工厂"这类做不到的
//   - 政策数字标出处页, 不做加总
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 3", "底座  ·  数据知识 · 传统系统的新位置", "P11 - P12", null],
    ["PART 4", "边界  ·  交付分工 · 接入规范 · 治理", "P13 - P15", null],
    ["PART 5", "建设内容 · 指标 · 申报口径", "P16 - P18", null],
    ["PART 6", "政策 · 实施 · 风险 · 下一步", "P19 - P22", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.9 + i * 0.86;
    if (pt[3]) {
...
// ═══════════════════════════════ P12 · 传统系统在 AI Native 里的位置 ★ 消除误解
{
  ...
  const rows = [
    ...
    ["视觉质检系统", "成像与缺陷判定",
      "由专业厂商成套交付", "输出缺陷数据供 Agent 归因（见 P13）"],
    ["报表与看板", "各类统计报表",
...
// ═══════════════════════════════ P16 · 建设内容清单
{
  ...
  const rows = [
    ...
    ["04", "工业网络与安全", "工业以太网、网络隔离、数据安全", "集成商", "技改"],
    ["05", "数据语义层", "统一语义模型、多形态存储、数据服务与权限", "本方", "技改（软件）"],
    ["06", "知识与记忆层", "行业知识库、企业知识库、案例与记忆、检索增强", "本方", "研发总部 / 关键技术"],
    ["07", "上下文工程", "三层上下文构建与持续维护机制", "本方", "关键技术攻关"],
    ["08", "Agent 编排平台", "意图理解、任务规划、工具调用、人在回路、留痕审计", "本方", "关键技术攻关"],
    ["09", "工具与技能库", "各业务工具封装、技能定义与沉淀机制", "本方", "技改（软件）"],
    ["10", "研发 Agent", "记录结构化、案例检索、需求受理、方向建议", "本方", "研发总部 / 功能性平台"],
    ["11", "生产 Agent", "批次串联、异常归因、参数建议、排产评估", "本方", "技改 · 关键技术"],
    ["12", "合规与客户 Agent", "清单解析组包、标准比对、碳足迹、客诉归因", "本方", "技改（软件）"],
    ["13", "园区 Agent", "受控技术咨询、产能协同、认证支持、企业接入", "本方", "区内协同采购（配套）"],
    ["14", "MES 与质量系统", "工单、排产、追溯记录、检验规则与判定", "本方", "技改（软件）"],
    ["15", "集成与实施服务", "接口开发、数据迁移、知识冷启动、培训试运行", "本方", "技改（服务）"],
  ];

  const y0 = 1.68, rh = 0.345;
  ...
  s.addText(
    "注 · 06-09 是 AI Native 架构特有的建设内容，传统信息化方案中没有对应项 —— 申报时的口径说明见 P18",
    {
...
// ═══════════════════════════════ P19 · 政策对接
{
  ...
  const rows = [
    ...
    ["05 14  数据层与 MES", "技术改造（软件投资）", "并入技改项目", "同上"],
    ["06 07 08  知识 / 上下文 / 编排", "支持关键核心技术突破", "新增投资 10-30% · 重点 3000 万", "填补国内空白"],
    ["06 10  知识库与研发 Agent", "设立研发总部 / 功能性平台", "项目总投资 50% · 最高 1000 万", "重点实验室等"],
    ["13  园区 Agent", "区内协同采购（平台提供数据支撑）", "采购发票额 10% · 1000 万 / 年", "双方无股权关联"],
  ...
  s.addText(
    "逐条摘自《上海临港新片区政策情况》p7-p12 · 以临港新片区管委会发布最新版本为准 · 申报主体为稻生万物 · 申报表述见 P18",
    {
...
// ═══════════════════════════════ P20 · 实施路径
{
  ...
  s.addText(
    "① 传感器与计量装置必须在产线建设期同步安装 —— 事后加装需停线改造（见 P14 / P16-02）\n" +
...
// ═══════════════════════════════ P21 · 风险
{
  ...
  const risks = [
    ...
    ["集成商交付不达标", "产", "数据接进来但缺点位、时间戳不同步，上层无法使用",
      "P14 验收标准作为付款节点前置条件 · 联调阶段本方参与"],
    ["期望管理失控", "AI", "「AI 什么都能干」的预期与实际能力落差，导致信任崩塌",
      "P15 明确不做什么 · 不承诺无数据支撑的效果 · 分期兑现建立信任"],
  ...
  risks.forEach((r, i) => {
    ...
    s.addShape(p.ShapeType.rect, { x: 0.8, y: y + 0.22, w: 0.3, h: 0.21, fill: { color: col } });
    s.addText(r[1] === "—" ? "·" : r[1], {
      x: 0.8, y: y + 0.225, w: 0.3, h: 0.2,
      fontSize: 7, bold: true, color: C.white, fontFace: "Calibri",
      align: "center", margin: 0,
    });
    s.addText(r[0], {
      x: 1.22, y: y + 0.08, w: 2.1, h: 0.5,
      fontSize: 10, bold: true, color: C.primary, fontFace: "Cambria",
      margin: 0, valign: "middle",
    });
    s.addText(r[2], {
      x: 3.45, y: y + 0.06, w: 4.85, h: 0.54,
...
p.writeFile({ fileName: "daosheng-v21-ai-native.pptx" })
  .then(() => console.log("✓ daosheng-v21-ai-native.pptx  ·  22 页"));
