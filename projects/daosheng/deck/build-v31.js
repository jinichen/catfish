...
// ═══════════════════════════════ P23 · 技术指标框架
{
  ...
  ["指标", "基线", "目标"].forEach((h, i) => {
    s.addText(h, {
      x: [0.75, 3.6, 5.1][i], y: 2.08, w: [2.7, 1.4, 1.4][i], h: 0.24,
  ...
  ["指标", "基线", "目标"].forEach((h, i) => {
    s.addText(h, {
      x: [6.9, 10.05, 11.45][i], y: 2.08, w: [3.0, 1.3, 1.2][i], h: 0.24,
      fontSize: 8.5, bold: true, color: C.white, fontFace: "Calibri", margin: 0,
    });
  });
  sup.forEach((c, i) => {
...
p.writeFile({ fileName: "daosheng-v31-smart-mfg.pptx" })
  .then(() => console.log("✓ daosheng-v31-smart-mfg.pptx  ·  24 页"));
