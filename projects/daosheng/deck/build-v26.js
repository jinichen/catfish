// 稻生万物研产销一体化产业园 · AI Native 数字化方案 · v26 · 26 页
//
// ═══ v26 vs v25 (7/28 鸿波第十四轮) ═══
//
// 鸿波: "申报的比对，让稻生万物去考虑，我们的方案里面不提"
//
// 边界再收一层: 申报是稻生万物作为申报主体的事, 与其在方案里替客户
// 推演政策口径（还是在缺少申报指南的情况下推演）, 不如干净地只交技术方案。
//
// ═══ 整页删除 ═══
//   申报口径翻译（AI Native → 政策语言）
//   临港政策对接（技改 / 首台套 / 研发总部 / 协同采购 / 贴息 / 所得税）
//
// ═══ 就地清理 11 页 ═══
//   目录       PART 名去掉"申报口径""政策"
//   部署拓扑   底注不再指向固投比例
//   部署形态   标题从"选型影响申报"改为纯技术经济对比;
//              三方案第四维从"投资性质(固投/服务费)"改为"弹性与扩展";
//              固投红框换成"配置核定的前置条件"
//   算力策略   三条代价删掉"固投比例", 剩数据位置与厂商锁定两条;
//              "现在就办"删掉核实固投口径与临港算力政策, 补模型实测与调用量预估
//   机房选型   概念澄清从"固投 vs 服务费"改为"运维与成本结构";
//              删临港政策核实注
//   建设清单   删"可对应政策"列, 换为"阶段"(与实施路径对应)
//   指标框架   从"申报评审按指标打分"改为"建设目标与验收依据";
//              不填估计值的理由从"资金追回与失信"改为"验收要按它交差"
//   风险       删"固投比例不达标"; "基线缺失"措辞去申报;
//              **补一条"开源模型能力不及预期"**(P14 已埋, 该进风险页)
//   下一步     第一步说明去掉"申报也报不了"
//
// 保留: 指标框架本身仍有价值 —— 它是建设目标与验收依据, 与申报无关也需要。
//
// ═══ 延续的结构性决定 ═══
//   ① AI Native 骨架: 交互 → Agent 编排 → 能力知识 → 数据语义 → 接入
//   ② 四个 Agent: 研发 / 生产 / 合规客户 / 园区
//   ③ 交付边界在数据接入层, 设备与视觉硬件交集成商, 现场零部署
//   ④ 开源模型自部署 + 租赁算力, 推理与向量数据不出自有实例
//   ⑤ 云优先是 PHASE 1 决策, 非永久架构
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - **全篇不出现申报 / 政策 / 补贴 / 固投 / 管委会**（禁塑政策等产业背景除外）
//   - 不点具体开源模型名, 不给参数量 / 显存 / 准确率
//   - 配置一律 [待核定] · 指标基线 [待实测] / 目标 [待定]
//   - 不写人月工期
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 5", "建设内容与验收指标", "P22 - P23", null],
    ["PART 6", "实施  ·  风险  ·  下一步", "P24 - P26", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.9 + i * 0.86;
    if (pt[3]) {
...
// ═══════════════════════════════ P22 · 建设内容清单
{
  ...
  const rows = [
    ...
    ["07", "数据语义层", "统一语义模型、多形态存储、数据服务与权限", "本方", "P1-P2"],
    ["08", "知识与记忆层", "行业知识库、企业知识库、案例与记忆、检索增强", "本方", "P1"],
    ["09", "上下文工程", "三层上下文构建与持续维护机制", "本方", "P1-P3"],
    ["10", "Agent 编排平台", "意图理解、任务规划、工具调用、人在回路、留痕审计", "本方", "P1-P3"],
    ...
    ["17", "集成与实施服务", "接口开发、数据迁移、知识冷启动、培训试运行", "本方", "P1-P3"],
  ];
...
// ═══════════════════════════════ P25 · 风险
{
  ...
  const risks = [
    ...
    ["基线缺失无法验收", "—", "指标基线未实测就定目标，验收时拿不出改造前数据做对比",
      "PHASE 1 首要任务即基线测评 · 采集方法见 P23"],
    ["开源模型能力不及预期", "AI", "实测发现关键任务（如标准差异解析、缺陷归因）开源模型啃不动",
      "选型阶段即在真实任务上实测（P14）· 保留脱敏后调用商业 API 的兜底路径，须经数据分级审批"],
  ];
  risks.forEach((r, i) => {
    const y = 1.7 + i * 0.66;
    s.addShape(p.ShapeType.rect, {
...
p.writeFile({ fileName: "daosheng-v26-tech-only.pptx" })
  .then(() => console.log("✓ daosheng-v26-tech-only.pptx  ·  26 页"));
