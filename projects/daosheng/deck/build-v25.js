// 稻生万物研产销一体化产业园 · AI Native 数字化方案 · v25 · 28 页
//
// ═══ v25 vs v24 (7/28 鸿波第十三轮) ═══
//
// 鸿波: "我的意思是推理、向量 都用开源模型，租赁算力来实现"
//
// 这比 v24 我理解的更准确, 而且**把最难办的那条代价大幅削弱了**。
//
// ═══ 关键区分：租算力 ≠ 用别人的模型服务 ═══
//
//   v24 P15 写"配方数据出厂"时, 默认的是调商业模型 API。但:
//     · 算力是通用 IaaS —— 租的是虚拟机与 GPU
//     · 模型是自己部署的 —— 推理数据只在自己控制的实例内,
//       **不经过任何模型服务商**
//
//   云厂商理论上可接触虚拟化层, 但这与"把 prompt 发给模型 API"是
//   完全不同量级的暴露面, 且可用加密与专有网络进一步收敛。
//
// ═══ 向量化这一环比推理侧更关键 ═══
//
//   企业知识库要把**全部**配方文档、工艺记录、检测报告做 embedding。
//   若走商业 embedding API, 等于把最敏感的资料全量发给第三方 ——
//   而且是一次性全量, 不是零星调用。
//   用开源 embedding 自部署, 这个问题根本不存在。
//
// ═══ 新增 P14「模型策略 · 开源自部署」═══
//   四条理由（数据主权 / 向量化尤其关键 / 成本可控 / 可迁移可微调）
//   → 模型分层（主力推理 / 向量嵌入 / 视觉理解 / 轻量抽取）
//   → **能力边界与实测要求**（不吹开源等于闭源）
//   → 运维复杂度与迭代机制
//
// ═══ 必须诚实写出的三个代价 ═══
//   ① 能力差距 —— 开源模型在复杂推理与长上下文上与顶尖闭源仍有差距,
//      须在真实任务上实测选型, 不预设结论
//   ② 运维复杂度 —— 推理框架、显存、并发、模型更新, 比调 API 复杂得多
//   ③ 成本拐点 —— 低频调用 API 更省, 高频/大量自部署更省; 需按实际调用量测算
//
// ═══ 连带修改四处口径 ═══
//   P13 拓扑    "通用大模型 API(可选)" → "自部署模型服务（租赁算力）"
//   P16 算力策略 "配方数据出厂"这条代价重写 —— 不再是主要顾虑
//   P18 安全    "核心配方不进外部模型" → 升级为"模型自部署, 推理数据不出实例"
//   P4 架构     能力与知识层体现自部署模型
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - **不点具体开源模型名** —— 避免显得已完成选型; 写"需实测选型"
//   - **不给模型参数量 / 显存需求 / 准确率** —— 未实测不臆测
//   - 配置一律 [待核定] · 固投口径标注需向管委会核实
//   - 政策数字标出处页, 不做加总
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 5", "建设内容 · 指标 · 申报口径", "P22 - P24", null],
    ["PART 6", "政策 · 实施 · 风险 · 下一步", "P25 - P28", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.9 + i * 0.86;
    if (pt[3]) {
...
// ═══════════════════════════════ P13 · 部署拓扑 ★
{
  ...
  s.addText(
    "注 · 平台承载区可选私有化 / 混合 / 公有云三种形态，选型直接影响投资结构与申报固投比例 —— 见 P15",
    {
...
// ═══════════════════════════════ P15 · 部署形态与算力 ★ 含固投比例提醒
{
  ...
  s.addText(
    "★ 申报关联 · 技改政策要求固定资产投资占比 ≥ 60%（见 P25）。若平台全部采用公有云订阅，该部分计为服务费而非固投，" +
...
// ═══════════════════════════════ P16 · 算力策略 · 当前阶段建议 ★
{
  ...
  const why = [
    ...
    ["AI 算力的特殊性", "自部署开源模型需 GPU 常驻；但采购周期长、贬值快，冷启动期负载极低易闲置，租赁可按需伸缩"],
  ];
  why.forEach((w, i) => {
    const y = 2.02 + i * 0.94;
    s.addShape(p.ShapeType.rect, {
...
// ═══════════════════════════════ P17 · 算力中心 · 自有机房 vs 租赁托管 ★
{
  ...
  subTitle(s, "当前阶段建议云优先（见 P16）· 本页为未来负载稳定后若转本地化的选型依据", C.redAccent);
...
// ═══════════════════════════════ P22 · 建设内容清单
{
  ...
  const rows = [
    ...
    ["05", "算力与存储", "PHASE 1 租赁 GPU 与通用算力；后续如转自购则为服务器", "选型本方", "云=服务费/自购=固投"],
    ["05c", "模型部署与调优", "开源推理/嵌入/视觉模型选型、部署、评测与迭代机制", "本方", "关键技术攻关"],
  ...
  s.addText(
    "注 · 05-06 为固定资产投资，直接关系技改申报的固投比例（见 P15）· 08-11 是 AI Native 特有内容，申报口径说明见 P24",
    {
...
// ═══════════════════════════════ P25 · 政策对接
{
  ...
  const rows = [
    ...
    ["05 06  算力与安全", "企业技术改造（固定资产投资）", "自购计固投 / 云订阅计服务费", "★ 口径待核实，见 P16"],
  ...
  ["对应建设内容（P22 编号）", "政策名称", "支持上限", "主要条件"].forEach((h, i) => {
  ...
  s.addText(
    "逐条摘自《上海临港新片区政策情况》p7-p12 · 以临港新片区管委会发布最新版本为准 · 申报主体为稻生万物 · 申报表述见 P24",
    {
...
p.writeFile({ fileName: "daosheng-v25-ai-native-oss.pptx" })
  .then(() => console.log("✓ daosheng-v25-ai-native-oss.pptx  ·  28 页"));
