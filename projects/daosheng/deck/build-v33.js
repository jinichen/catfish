// 稻生万物智能制造建设方案 · v31 · 24 页
//
// ═══ v31 vs v30 (7/28 鸿波第十九轮) ═══
//
// 鸿波: "第四页的块出界了，我再去手动改很麻烦"
//
// 不靠眼看, 直接算所有形状的右边界 —— 查出 5 处真越界:
//
//   p4  架构页卡片   超版心 0.40"   卡片起点 2.60 但可用宽按 10.50 算 → 冲到 13.10
//   p13 拓扑页卡片   超版心 0.40"   同一个 bug: 起点 3.25 / 宽度按 9.85 算
//   p17 「占优」表头  超版心 2.20"   x=11.9 却写死 w=3 → 右边界 14.90（已出画布）
//   p22 「阶段」表头  超版心 0.90"   x=10.6 写死 w=3
//   p23 「目标」表头  超版心 0.35"   x=11.45 写死 w=1.6
//
// 根因两类:
//   ① 卡片布局的"可用宽度"是手填的常数, 与实际起点/底板宽度对不上
//      → 改为从底板右界倒算: avail = 12.70 - 起点 - 0.15(右留白)
//   ② 表头文本框统一写 w:3, 末列必然溢出 → 按列实际宽度给
//
// ═══ 固化检查, 不再靠人眼 ═══
//
// 新增 check-geometry.py: 读生成的 pptx, 算每个形状的右/下边界,
// 超版心即报错并列出页码与内容。build 后必须跑。
//   版心: x 0.60–12.70 · y 0.50–7.45（页脚 7.15 之下留白）
//   排除: x≈0 且 w≈满幅 的装饰条（封面顶部金条是有意满幅）
//
// 顺带把页脚页码框从 x=W-1.2(右界 12.80) 收到 12.70, 与版心齐。
//
// ═══ 延续的结构性决定 ═══
//   ① AI Native 骨架 ② 四个 Agent ③ 交付边界在数据接入层, 现场零部署
//   ④ 开源模型自部署 + 租赁算力 ⑤ 云优先是 PHASE 1 决策
//   ⑥ 不提申报 / 政策 / 补贴 ⑦ 不设「风险与应对」「下一步」
//   ⑧ 计算化学由外部机构承担 ⑨ 标签自带意思, 不靠注解兜底
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - 标签自带意思, 不靠注解兜底
//   - **布局宽度一律倒算, 不写死常数**
//   - **build 后必跑 check-geometry.py** —— 越界不算完成
//   - 不出现申报 / 政策 / 补贴 / 固投 / 管委会
//   - 不点具体开源模型名 · 配置 [待核定] · 指标 [待实测]/[待定]
//
// 数据来源:
...
function footer(s) {
  s.addText("稻生万物智能制造建设方案  ·  研产销一体化产业园  ·  上海临港新片区", {
    x: 0.6, y: 7.15, w: 11, h: 0.3,
    fontSize: 9, color: C.grayLt, fontFace: "Calibri", margin: 0,
  });
  s.addText(`${PN} / ${TOTAL}`, {
    x: W - 1.33, y: 7.15, w: 0.73, h: 0.3,
...
function agentPage(s, { serves, inputs, tasks, human, prereq, phase, color }) {
  ...
  tasks.forEach((t, i) => {
    ...
    s.addShape(p.ShapeType.rect, {
      x: 5.2, y, w: 7.5, h: 0.72,
      fill: { color: C.white }, line: { color: C.bg2, width: 1 },
    });
    s.addShape(p.ShapeType.rect, { x: 5.2, y, w: 0.05, h: 0.72, fill: { color: color } });
    s.addText(t[0], {
      x: 5.42, y: y + 0.07, w: 2.9, h: 0.26,
      fontSize: 10.2, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
    });
    s.addText(t[1], {
      x: 5.42, y: y + 0.33, w: 7.05, h: 0.36,
      fontSize: 8.3, color: C.gray, fontFace: "Calibri",
      lineSpacing: 11, margin: 0, valign: "top",
    });
    if (t[2]) {
      const dc = t[2] === "低" ? C.secondary : t[2] === "高" ? C.redAccent : C.accent;
      s.addShape(p.ShapeType.rect, { x: 12.0, y: y + 0.07, w: 0.55, h: 0.22, fill: { color: dc } });
      s.addText(t[2], {
        x: 12.0, y: y + 0.075, w: 0.55, h: 0.21,
  ...
  // 底 · 分期
  s.addShape(p.ShapeType.roundRect, {
    x: 5.2, y: 6.12, w: 7.5, h: 0.56,
    fill: { color: C.white }, line: { color: color, width: 1.5 }, rectRadius: 0.05,
  });
  s.addText(phase[0], {
    x: 5.42, y: 6.22, w: 1.9, h: 0.28,
    fontSize: 10, bold: true, color: color, fontFace: "Cambria", margin: 0,
  });
  s.addText(phase[1], {
    x: 7.4, y: 6.24, w: 5.1, h: 0.28,
...
// ═══════════════════════════════ P4 · AI Native 总体架构 ★
{
  ...
  let y = 1.7;
  layers.forEach((L, li) => {
    const h = li === 1 ? 0.88 : (li === 2 ? 0.94 : 0.82);
    const isAgent = li === 1;
    s.addShape(p.ShapeType.roundRect, {
      x: 0.6, y, w: 12.1, h,
    ...
    const n = L[3].length;
    // 可用宽 = 底板右界 12.70 − 卡片起点 2.60 − 右留白 0.15
    const cw = (9.95 - (n - 1) * 0.12) / n;
    L[3].forEach((it, i) => {
...
// ═══════════════════════════════ P7 · 研发 Agent
{
  ...
  s.addText(
    ...
    {
      x: 0.6, y: 6.74, w: 12.1, h: 0.34,
      fontSize: 7.6, color: C.accentDk, fontFace: "Calibri",
      lineSpacing: 11, margin: 0, valign: "top",
    }
  );
  footer(s);
...
// ═══════════════════════════════ P13 · 部署拓扑 ★
{
  ...
  zones.forEach((z, zi) => {
    ...
    const n = z[2].length;
    // 可用宽 = 底板右界 12.70 − 卡片起点 3.25 − 右留白 0.15
    const cw = (9.3 - (n - 1) * 0.12) / n;
    z[2].forEach((it, i) => {
    ...
    if (zi < 3) {
      const labels = ["工业防火墙 / 网闸  ·  OT-IT 隔离",
                      ...
                      "外部边界  ·  防火墙 · 认证 · 审计"];
      s.addShape(p.ShapeType.rect, {
        x: 4.4, y: y + h + 0.02, w: 4.5, h: 0.16,
        fill: { color: C.redAccent },
      });
      s.addText(labels[zi], {
        x: 4.4, y: y + h + 0.028, w: 4.5, h: 0.15,
        fontSize: 6.8, bold: true, color: C.white, fontFace: "Calibri",
        align: "center", margin: 0,
      });
    }
    y += h + 0.20;
...
// ═══════════════════════════════ P14 · 模型策略 · 开源自部署 ★
{
  ...
  tiers.forEach((t, i) => {
    ...
    s.addShape(p.ShapeType.rect, { x: 0.6, y, w: 0.05, h: 0.46, fill: { color: t[1] } });
    s.addText(t[0], {
  ...
  s.addShape(p.ShapeType.roundRect, {
    x: 8.9, y: 6.24, w: 3.8, h: 0.82,
    fill: { color: C.cream }, line: { color: C.bg2, width: 1 }, rectRadius: 0.05,
  });
  s.addText("运维与迭代", {
    x: 9.12, y: 6.33, w: 3.4, h: 0.24,
    fontSize: 10, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
  });
  s.addText(
...
// ═══════════════════════════════ P16 · 算力策略 · 当前阶段建议 ★
{
  ...
  why.forEach((w, i) => {
    const y = 2.00 + i * 0.88;
    s.addShape(p.ShapeType.rect, {
      x: 0.6, y, w: 5.95, h: 0.80,
      fill: { color: C.white }, line: { color: C.bg2, width: 1 },
    });
    s.addShape(p.ShapeType.rect, { x: 0.6, y, w: 0.05, h: 0.80, fill: { color: C.secondary } });
    s.addText(w[0], {
  ...
  // 底 · 分阶段路线 + 现在就要办的三件事
  s.addShape(p.ShapeType.roundRect, {
    x: 0.6, y: 6.46, w: 12.1, h: 0.62,
    fill: { color: C.primary }, line: { width: 0 }, rectRadius: 0.05,
  });
  s.addText("分阶段", {
    x: 0.85, y: 6.53, w: 1.1, h: 0.24,
...
// ═══════════════════════════════ P17 · 算力中心 · 自有机房 vs 租赁托管 ★
{
  ...
  ["安全维度", "自有机房", "租赁托管", "占优"].forEach((h, i) => {
    ...
    const ws = [2.6, 3.8, 4.35, 0.75];
    s.addText(h, {
      x: xs[i], y: y0 + 0.05, w: ws[i], h: 0.22,
...
// ═══════════════════════════════ P22 · 建设内容清单
{
  ...
  const rows = [
    ...
    ["06", "机房或托管（后续）", "转本地化时：自建土建配套 或 托管机柜与专线", "建设方", "P3"],
    ["07", "安全设备与等保建设", "边界防护、身份认证、审计、等保测评整改", "选型本方", "P1"],
    ["08", "数据语义层", "全域统一定义、多形态存放、统一取数与权限", "本方", "P1-P2"],
    ["09", "知识与记忆层", "行业知识库、企业知识库、案例与记忆、检索增强", "本方", "P1-P3"],
    ["10", "上下文工程", "三层上下文构建与持续维护机制", "本方", "P1-P3"],
    ["11", "模型部署与调优", "开源推理/嵌入/视觉模型选型、部署、评测与迭代机制", "本方", "P1-P3"],
    ["12", "Agent 编排平台", "听懂要做什么、拆解编排、调用工具、交人确认、留痕审计", "本方", "P1-P3"],
    ["13", "工具与技能库", "各业务工具封装、技能定义与沉淀机制", "本方", "P1-P2"],
    ["14", "研发 Agent", "记录结构化、案例检索、需求受理、方向建议", "本方", "P1"],
    ["15", "生产 Agent", "批次串联、异常归因、参数建议、排产评估", "本方", "P2-P3"],
    ["16", "合规与客户 Agent", "清单解析组包、标准比对、碳足迹、客诉归因", "本方", "P1-P2"],
    ["17", "园区 Agent", "受控技术咨询、产能协同、认证支持、企业接入", "本方", "P3"],
    ["18", "MES 与质量系统", "工单、排产、追溯记录、检验规则与判定", "本方", "P1-P2"],
    ["19", "集成与实施服务", "接口开发、数据迁移、先把已有资料灌进知识层、培训试运行", "本方", "P1-P3"],
  ];

  const y0 = 1.60, rh = 0.253;
  s.addShape(p.ShapeType.rect, { x: 0.6, y: y0, w: 12.1, h: 0.26, fill: { color: C.primary } });
  ["", "建设内容", "主要构成", "实施方", "阶段"].forEach((h, i) => {
    const xs = [0.75, 1.2, 3.9, 9.1, 10.6];
    const ws = [0.4, 2.6, 5.1, 1.4, 2.0];
    if (!h) return;
    s.addText(h, {
      x: xs[i], y: y0 + 0.04, w: ws[i], h: 0.22,
      fontSize: 9, bold: true, color: C.white, fontFace: "Calibri", margin: 0,
    });
  });
  rows.forEach((r, i) => {
    const y = y0 + 0.26 + i * rh;
    s.addShape(p.ShapeType.rect, {
      x: 0.6, y, w: 12.1, h: rh,
      fill: { color: i % 2 ? C.white : C.cream }, line: { color: C.bg2, width: 0.5 },
    });
    s.addText(r[0], {
      x: 0.75, y: y + 0.04, w: 0.4, h: 0.2,
      fontSize: 7.6, bold: true, color: C.accent, fontFace: "Cambria", margin: 0,
    });
    s.addText(r[1], {
      x: 1.2, y: y + 0.035, w: 2.6, h: 0.22,
      fontSize: 8.2, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
    });
    s.addText(r[2], {
      x: 3.9, y: y + 0.045, w: 5.1, h: 0.2,
      fontSize: 7.1, color: C.gray, fontFace: "Calibri", margin: 0,
    });
    const isOurs = r[3] === "本方";
    s.addShape(p.ShapeType.rect, {
      x: 9.1, y: y + 0.045, w: 1.05, h: 0.19,
      fill: { color: isOurs ? C.primary : C.grayLt },
    });
    s.addText(r[3], {
      x: 9.1, y: y + 0.055, w: 1.05, h: 0.18,
...
// ═══════════════════════════════ P23 · 验收指标（只列本方能负责的）
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "验收指标", "PART 5 · ACCEPTANCE KPI");
  subTitle(s, "只列本方案能负责的 —— 产线效益指标由设备与工艺决定，不在本方案承诺范围", C.redAccent);

  const cols = [
    ["建成了什么  ·  能力覆盖", C.ai, [
      ["关键工序数据采集覆盖率", "已接入点位 / 应接入点位"],
      ["质量追溯覆盖率", "可正反查批次 / 总批次"],
      ["知识资产化率", "已结构化沉淀 / 应沉淀的工艺与案例"],
      ["Agent 任务覆盖率", "由 Agent 承担 / 可自动化的业务任务"],
      ["回答可溯源率", "附出处的回答 / 全部专业类回答"],
    ]],
    ["用起来没有  ·  实际使用", C.sales, [
      ["建议采纳率", "人工采纳 / Agent 给出的建议总数"],
      ["一线活跃使用率", "实际使用人数 / 应使用人数"],
      ["知识复用率", "被检索调用过的条目 / 已沉淀条目"],
      ["下游自助解决率", "入驻企业自助完成 / 全部服务请求"],
      ["人工返工率", "需人工大幅修改的输出 / 全部输出"],
    ]],
  ];
  ...
  cols.forEach((col, ci) => {
    const x = 0.6 + ci * 6.15;
    s.addText(col[0], {
      x, y: 1.70, w: 5.95, h: 0.28,
      fontSize: 11.5, bold: true, color: col[1], fontFace: "Cambria", margin: 0,
    });
    s.addShape(p.ShapeType.rect, { x, y: 2.04, w: 5.95, h: 0.3, fill: { color: col[1] } });
    ["指标", "怎么算", "目标"].forEach((h, i) => {
      s.addText(h, {
        x: [x + 0.15, x + 2.5, x + 5.0][i], y: 2.08, w: [2.2, 2.4, 0.8][i], h: 0.24,
    ...
    col[2].forEach((r, i) => {
      ...
      s.addText(r[0], {
        x: x + 0.15, y: y + 0.16, w: 2.3, h: 0.3,
        fontSize: 9.5, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
      });
      s.addText(r[1], {
        x: x + 2.5, y: y + 0.17, w: 2.45, h: 0.28,
        fontSize: 7.6, color: C.gray, fontFace: "Calibri", margin: 0,
      });
      s.addText("[待定]", {
        x: x + 5.0, y: y + 0.17, w: 0.85, h: 0.28,
        fontSize: 8.2, bold: true, color: C.grayLt, fontFace: "Calibri", margin: 0,
      });
    });
  });

  s.addShape(p.ShapeType.roundRect, {
    x: 0.6, y: 5.55, w: 12.1, h: 1.44,
    fill: { color: "FDF3F2" }, line: { color: C.redAccent, width: 1.5 }, rectRadius: 0.05,
  });
  s.addText("为什么没有「生产效率 / 良品率 / 单位能耗」这类指标", {
    x: 0.9, y: 5.66, w: 8, h: 0.28,
    fontSize: 11, bold: true, color: C.redAccent, fontFace: "Cambria", margin: 0,
  });
  s.addText(
    "这些效益指标由产线设备选型与工艺水平决定。本方案现场零部署、不承担产线建设（见 P19），" +
    "数字化能为其提供数据支撑与归因依据，但不单独承诺数值 —— 写进来就等于替产线的效果背书。\n" +
    "上表指标的基线多为零（属新建），目标值需与最终建设范围一并商定，本页不预设。",
    {
      x: 0.9, y: 5.98, w: 11.5, h: 0.92,
      fontSize: 8.8, color: C.gray, fontFace: "Calibri",
      lineSpacing: 14, margin: 0, valign: "top",
    }
  );
...
// ═══════════════════════════════ P24 · 实施路径
{
  ...
  s.addText(
    ...
    {
      x: 0.9, y: 6.25, w: 11.5, h: 0.52,
      fontSize: 8.5, color: C.gray, fontFace: "Calibri",
      lineSpacing: 13, margin: 0, valign: "top",
    }
  );
  s.addText("注 · 各阶段时长需结合产线建设周期确定，本页不列具体月份", {
    x: 0.6, y: 6.82, w: 12.1, h: 0.26,
    fontSize: 8, color: C.grayLt, fontFace: "Calibri", margin: 0,
  });
  footer(s);
}

p.writeFile({ fileName: "daosheng-v33-smart-mfg.pptx" })
  .then(() => console.log("✓ daosheng-v33-smart-mfg.pptx  ·  24 页"));
