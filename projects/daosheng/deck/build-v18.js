// 稻生万物研产销一体化产业园 · 数字化技术方案 · v18 · 21 页
//
// ═══ v18 vs v17 (7/28 鸿波第六轮) ═══
//
// 鸿波: "要有研产销"
//
// v17 的毛病 —— 场景全压在"产"上:
//   追溯 / 视觉质检 / 工艺寻优 / 能碳 四个都是生产侧, 只有园区协同跨了一点.
//   但园区叫「研产销一体化」, 数字化只做生产, 等于把方案的骨架砍掉三分之二.
//   而且"研"和"销"恰恰是这个园区区别于普通工厂的地方 —— 链主要向下游输出
//   技术(研)和渠道(销), 没有数字化支撑就只能靠人肉.
//
// ═══ v18 场景重组 · 按研产销分组 ═══
//
//   研  P7  研发数据管理与工艺知识库    把经验变成可检索、可受控输出的资产
//       P8  新品开发协同               下游提需求 → 配方 → 中试 → 量产 的链路
//   产  P9  生产执行与全流程追溯        地基, 其它场景都建在批次链上
//       P10 质量与工艺闭环             视觉质检 + 缺陷参数关联 + 参数寻优 (v17 两页合一,
//                                      因为它们本来就是一个闭环: 发现缺陷→找原因→调参数)
//   销  P11 认证合规与碳足迹            认证档案 + 审厂材料 + CBAM 核算
//       P12 客户订单与交付协同          客户要求结构化 + 进度可见 + 园区拆单
//   跨  P13 园区协同平台                把研产销三段的能力向入驻企业开放
//
// 连带调整:
//   P2 背景  —— 数字化在研/产/销各段分别承担什么 (不再是笼统四条)
//   P4 架构  —— 应用层按研产销分组, 与园区结构同构
//   P5 数据流 —— 三类数据源 (研发侧/生产侧/销售侧), v17 只有设备数据
//   P15 建设清单 —— 补研发侧与销售侧系统, 从 10 项增至 14 项
//
// ═══ 延续 v17 的两个结构性决定 ═══
//   ① 产线在 P6「接入前提」出现, 不作为建设内容 —— 产线由建设方与设备商定
...
// ═══════════════════════════════ P1 · 封面
{
  ...
  s.addText(
    "产线设备选型与工艺路线由建设方与设备供应商确定，本方案给出接入条件要求",
    {
      ...
      fontSize: 10, color: C.secondary, fontFace: "Calibri", margin: 0,
    }
  );
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ["PART 1", "总体设计  ·  架构 / 数据流 / 接入前提", "P4 - P6", null],
    ["PART 2", "研  ·  研发数据与新品协同", "P7 - P8", C.rd],
    ["PART 3", "产  ·  生产执行与质量工艺", "P9 - P10", C.mfg],
    ["PART 4", "销  ·  认证合规与客户交付", "P11 - P12", C.sales],
    ["PART 5", "园区协同 · 集成 · 建设内容 · 指标", "P13 - P16", null],
    ["PART 6", "政策申报  ·  实施  ·  风险", "P17 - P21", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.85 + i * 0.85;
    if (pt[3]) {
...
// ═══════════════════════════════ P4 · 总体架构
{
  ...
  s.addText("把研产销三段的能力按授权范围向入驻企业开放（见 P13）", {
    x: 4.3, y: 5.95, w: 8.2, h: 0.28,
    fontSize: 9, color: C.cream, fontFace: "Calibri", margin: 0,
  });

  s.addText(
    "两条设计原则 · 边缘自治：断网时采集与质检不停，恢复后补传    ·    单一数据源：应用不直连设备，统一走数据服务 API 取数",
    {
...
// ═══════════════════════════════ P5 · 数据流 · 三类数据源
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "数据流  ·  三类数据源汇聚", "PART 1 · DATA FLOW");
...
// ═══════════════════════════════ P6 · 接入前提
{
  ...
  const groups = [
    ...
    ["现场条件", C.steel, [
      ...
      ["中试线数据接口", "中试设备同样需要采集能力，否则研发数据仍靠人工填表"],
    ]],
    ["既有系统与数据", C.rd, [
      ["ERP / 财务系统接口", "物料主数据、订单、成本科目的读取接口或数据库视图"],
      ["历史数据", "近 12 个月产量、质检、能耗记录 —— 用于建立指标基线（见 P16）"],
      ["研发与认证资料", "既有实验记录、配方文档、认证证书与检测报告的归集整理"],
    ]],
  ];

  let cy = 1.7;
  groups.forEach((g) => {
    s.addShape(p.ShapeType.rect, { x: 0.6, y: cy, w: 12.1, h: 0.3, fill: { color: g[1] } });
    s.addText(g[0], {
      x: 0.78, y: cy + 0.03, w: 3, h: 0.24,
      fontSize: 9.5, bold: true, color: C.white, fontFace: "Cambria", margin: 0,
    });
    cy += 0.3;
    g[2].forEach((r, i) => {
      s.addShape(p.ShapeType.rect, {
        x: 0.6, y: cy, w: 12.1, h: 0.46,
        fill: { color: i % 2 ? C.white : C.cream }, line: { color: C.bg2, width: 0.5 },
      });
      s.addText(r[0], {
        x: 0.85, y: cy + 0.1, w: 3.3, h: 0.28,
        fontSize: 9.5, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
      });
      s.addText(r[1], {
        x: 4.3, y: cy + 0.11, w: 8.2, h: 0.28,
...
// ═══════════════════════════════ P7 · 研 · 研发数据管理与工艺知识库
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "研发数据管理与工艺知识库", "PART 2 · R&D DATA", C.rd, "研");
...
// ═══════════════════════════════ P8 · 研 · 新品开发协同
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "新品开发协同", "PART 2 · NPD COLLABORATION", C.rd, "研");
  ...
  scenarioPage(s, {
    ...
    steps: [
      ...
      ["复盘归档", "成功与失败案例一并入库，与 P7 知识库打通供后续检索"],
    ],
    needs: [
      "开发流程需先梳理定义 —— 有没有阶段门是管理决策，系统只是承载",
      "中试线具备数据采集条件（见 P6 现场条件）",
      "下游企业配合使用需求表单；可在入驻协议中约定",
      "研发与生产的工艺卡格式需统一，否则转移仍要人工翻译",
    ],
    phase: ["PHASE 2 · 中试线就绪后", "可与 P7 知识库同期建设；需求受理部分可更早上线"],
    phaseColor: C.rd,
  });
  footer(s);
}
...
// ═══════════════════════════════ P9 · 产 · 生产执行与全流程追溯
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "生产执行与全流程追溯", "PART 3 · MES & TRACEABILITY", C.mfg, "产");
...
// ═══════════════════════════════ P10 · 产 · 质量与工艺闭环
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "质量与工艺闭环", "PART 3 · QUALITY & PROCESS", C.mfg, "产");
...
// ═══════════════════════════════ P11 · 销 · 认证合规与碳足迹
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "认证合规与碳足迹", "PART 4 · COMPLIANCE & CARBON", C.sales, "销");
  ...
  scenarioPage(s, {
    ...
    needs: [
      "分表分项计量在产线建设期同步预留（见 P6）—— 没有分表就没有碳足迹",
      "批次链已建立（P9），否则能耗无法分摊到产品",
...
// ═══════════════════════════════ P12 · 销 · 客户订单与交付协同
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "客户订单与交付协同", "PART 4 · ORDER & DELIVERY", C.sales, "销");
  ...
  scenarioPage(s, {
    ...
    steps: [
      ...
      ["投诉闭环", "客诉关联到批次、工艺参数与配方版本，回流研发（P7）形成改进闭环"],
    ],
    needs: [
      "MES 与批次链已建成（P9），否则进度与追溯数据无来源",
      "对客户的开放范围由商务协议约定 —— 哪些可见、哪些不可见是商务决策",
      "入驻企业接入园区平台（P13），拆单协同才有对象",
...
// ═══════════════════════════════ P13 · 园区协同平台
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "园区协同平台  ·  把研产销向入驻企业开放", "PART 5 · PARK PLATFORM");
  ...
  const pts = [
    ...
    ["采购结算数据归集", "区内交易的合同与发票数据归集，直接支撑协同采购补贴申报（见 P17）"],
...
// ═══════════════════════════════ P14 · 集成边界
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "系统集成边界  ·  和谁接、接什么", "PART 5 · INTEGRATION");
  subTitle(s, "把接口关系写清楚，避免交付时出现「我以为对方负责」");

  const rows = [
    ["产线设备 / PLC", "产", "工艺参数、设备状态、产量计数、报警", "设备供应商提供协议文档与点位表"],
    ["检测仪器 / 中试线", "研 产", "在线检测值、中试试制数据", "仪器需具备数字输出接口"],
    ["计量装置", "产 销", "电 / 气 / 水分项读数（碳足迹底数）", "需支持远传（RS485 / 网口）"],
    ["视觉质检设备", "产", "图像采集与判定结果", "相机与光源随产线安装"],
    ["ERP / 财务", "产 销", "物料主数据、订单、成本科目 / 完工与耗用回写", "客户 IT 或 ERP 供应商配合开接口"],
    ["检测机构 / 认证机构", "研 销", "检测报告、证书与有效期", "多为人工归集，可先做结构化录入"],
    ["入驻企业系统", "研 产 销", "需求、订单、库存、产能（按授权范围）", "有系统走接口 / 无系统用 Web 端"],
    ["品牌客户", "销", "订单进度与质量追溯（受限查询）", "开放范围由商务协议约定"],
    ["政府 / 监管报送", "销", "能耗、碳排、安全等合规数据", "按主管部门要求的格式与频率"],
  ];
...
// ═══════════════════════════════ P15 · 建设内容清单
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "建设内容清单  ·  数字化侧", "PART 5 · SCOPE OF WORK");
  subTitle(s, "仅列数字化范围 · 产线设备与厂房不在此表 · 投资金额待选型询价后概算");

  const rows = [
    ["01", "边缘采集与联网", "产", "协议网关、边缘节点、采集代理、工位终端", "技改（智能化）"],
    ["02", "传感与计量装置", "产 销", "补充传感器、分项计量表具及远传模块", "技改 · 绿色制造"],
    ["03", "工业网络与安全", "产", "工业以太网、网络隔离、数据安全与权限", "技改"],
    ["04", "数据平台", "研 产 销", "时序库、关系库、对象存储、数据服务 API", "技改（软件）"],
    ["05", "MES 与批次追溯", "产", "工单、排产、追溯、质量、设备与模具管理", "技改（软件）"],
    ["06", "视觉质检系统", "产", "成像单元、标注平台、模型训练与推理部署", "技改 · 关键技术"],
    ["07", "研发数据管理", "研", "实验记录、配方版本、检测数据归集", "研发总部 / 功能性平台"],
    ["08", "工艺知识库与寻优", "研 产", "参数结构化、知识检索、关联分析与推荐模型", "关键技术攻关"],
    ["09", "新品开发协同", "研", "需求受理、阶段门、中试回流、量产转移", "研发总部 / 功能性平台"],
    ["10", "认证合规档案", "销", "证书与报告归档、有效期预警、审厂组包", "技改（软件）"],
    ["11", "能碳与碳足迹", "产 销", "能耗采集、核算模型、报告生成与报送", "绿色制造 / 技改"],
    ["12", "客户订单与交付", "销", "客户要求库、进度可视、交付文档生成", "技改（软件）"],
    ["13", "园区协同平台", "研 产 销", "共享看板、协同订单、企业接入端、结算归集", "区内协同采购（配套）"],
    ["14", "集成与实施服务", "—", "接口开发、数据迁移、部署调试、培训试运行", "技改（服务）"],
  ];

  const y0 = 1.68, rh = 0.365;
  s.addShape(p.ShapeType.rect, { x: 0.6, y: y0, w: 12.1, h: 0.3, fill: { color: C.primary } });
  ["", "建设内容", "涉及", "主要构成", "可对应政策"].forEach((h, i) => {
    const xs = [0.75, 1.2, 3.6, 4.85, 10.2];
    if (!h) return;
    s.addText(h, {
  ...
  rows.forEach((r, i) => {
    ...
    s.addText(r[3], {
      x: 4.85, y: y + 0.07, w: 5.25, h: 0.24,
      fontSize: 8, color: C.gray, fontFace: "Calibri", margin: 0,
    });
    s.addText(r[4], {
      x: 10.2, y: y + 0.07, w: 2.4, h: 0.24,
      fontSize: 7.8, bold: true, color: C.accentDk, fontFace: "Calibri", margin: 0,
    });
  });

  s.addText(
    "注 · 02 项虽属硬件但与数字化直接绑定（不装表就没有能耗数据），建议与产线同期实施 · 07/09 属研发属性，可对应研发类政策",
    {
...
// ═══════════════════════════════ P17 · 政策对接
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "临港政策对接  ·  按建设内容匹配", "PART 6 · POLICY MAP");
  ...
  ["对应建设内容（P15 编号）", "政策名称", "支持上限", "主要条件"].forEach((h, i) => {
...
// ═══════════════════════════════ P18 · 申报要件对照
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "申报要件对照  ·  谁准备什么", "PART 6 · FILING CHECKLIST");
  subTitle(s, "申报主体为稻生万物 · 下列为通用要件，具体以当年申报指南为准", C.redAccent);

  const rows = [
    ["技术改造和智能化升级", "可研报告 · 投资明细 · 软硬件合同 · 技改前后指标对比",
      "技术方案 · 架构与集成设计 · 建设清单（P15）· 指标框架（P16）", "财务报表 · 固投证明 · 合同与发票"],
...
// ═══════════════════════════════ P19 · 实施路径
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "实施路径  ·  按技术依赖排序", "PART 6 · ROADMAP");
  subTitle(s, "不按价值大小排，按依赖关系排 —— 地基没打好，上面的场景做了也是返工");

  const phases = [
    ["PHASE 1", "地基", [
      ["产", "点位表梳理 · 设备联网与数采", C.mfg],
      ["产", "分项计量装置 · MES 与批次追溯", C.mfg],
  ...
  s.addText(
    "① 传感器与计量装置必须在产线建设期同步安装 —— 事后加装需停线改造，成本与风险显著上升（见 P6 / P15-02）\n" +
...
p.writeFile({ fileName: "daosheng-v18-digital-rps.pptx" })
  .then(() => console.log("✓ daosheng-v18-digital-rps.pptx  ·  21 页"));
