// 稻生万物研产销一体化产业园 · 数字化技术方案 · v20 · 24 页
//
// ═══ v20 vs v19 (7/28 鸿波第八轮) ═══
//
// 鸿波两问: "没有提到 AI 的部分" + "是否一定要介入边缘采集？"
//
// 这两问是同一个答案 —— **AI 恰好是不碰边缘也能做的那部分, 而且它才是价值区**。
//
// ═══ 问题二：边缘采集不必介入, 而且 v19 还有一处漏网 ═══
//
// v19 已把边缘层交给集成商, 但 P11 仍写着"视觉质检模型部署在边缘节点" ——
// 那是实打实的现场介入: 车间装推理硬件、调成像、扛粉尘震动、保 7×24。
// 机器视觉是成熟行业(海康/凌云/康耐视等有成套方案), 自己做没优势还背风险。
//
// v20 改法: **判定交给视觉厂商, 我方接判定结果做归因分析**。
//   厂商交付: 相机/光源/推理硬件/基础缺陷判定 → 输出缺陷类型与图像
//   我方交付: 缺陷数据接入 → 与工艺参数/原料特性关联 → 归因 → 参数推荐
//   "这批缺陷是哪个参数导致的" 才是没人做、也只有拿到全量数据才能做的事。
//
// 至此我方在现场**零部署**: 全部能力跑在平台层。
//
// ═══ 问题一：AI 缺页是硬伤 ═══
//
// 若既不碰设备又不讲 AI, 我方就只是个做管理软件的 —— 谁都能替代。
// v19 里 AI 只零星散在 P11/P8, 没有独立呈现, 差异化立不住。
//
// 新增两页:
//   P14 AI 能力地图      按研产销列能力 · 标难度 · 标数据前提
//   P15 AI 落地路径与边界 分期(先文档后数据后模型) + **明确不做什么**
//
// P15 的"不做什么"比"能做什么"更重要 —— AI 在方案里被吹滥了,
// 写清楚边界(不做无人自动调参 / 不做产线实时控制 / 不做质检最终裁决)
// 反而是可信度的来源, 也和"不碰边缘"的定位一致。
//
// AI 能力的难度分级依据: 对既有能力的真实盘点(工具链 / 技能凝固闭环 /
// MCP 接入 / 多模型路由 / 定时任务 / 视觉理解), 不是按宣传口径排。
//
// ═══ 延续的结构性决定 ═══
//   ① 交付边界在"数据接入层"切一刀, 接口规范仍由我方定义 (P4)
//   ② 场景按研产销分组: 研 2 / 产 2 / 销 2 / AI 2 / 跨 1
...
//   - 不写人月工期
//   - 指标基线 [待实测] / 目标 [待定]
//   - **AI 效果不承诺数字** —— 准确率/提升幅度一律不写, 只写数据前提与难度
//   - 政策数字标出处页, 不做加总
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 6", "AI 能力  ·  地图 / 落地路径 / 边界", "P14 - P15", C.accent],
    ["PART 7", "园区协同 · 集成 · 建设内容 · 指标", "P16 - P19", null],
    ["PART 8", "政策申报  ·  实施  ·  风险", "P20 - P24", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.8 + i * 0.66;
    if (pt[3]) {
...
// ═══════════════════════════════ P4 · 交付边界与分工 ★ v19 核心
{
  ...
  const bands = [
    ["应用层", C.secondary, "MES · 知识库 · 新品协同 · 认证档案 · 订单交付 · 园区协同 · AI 分析与推荐",
      "本方案范围", true],
    ...
    ["边缘与网络层", C.steel, "协议网关 · 边缘节点 · 采集代理 · 工业以太网 · 网络隔离",
      "系统集成商实施", false],
    ["设备与现场层", C.mfg, "PLC · 传感器与计量表具 · 视觉质检成套（相机/光源/推理硬件/判定）· 布线安装",
      "设备供应商 + 系统集成商", false],
  ];
  ...
  s.addText(
    ...
    "视觉质检亦交专业厂商成套交付 —— 本方在现场零部署，全部能力跑在平台层。",
    {
...
// ═══════════════════════════════ P5 · 总体架构
{
  ...
  s.addText("把研产销三段的能力按授权范围向入驻企业开放（见 P16）", {
...
// ═══════════════════════════════ P11 · 产 · 质量与工艺闭环
{
  ...
  subTitle(s, "检测交专业厂商、归因与调参由本方 —— 分工不同但必须闭环，否则只治标");
  ...
  scenarioPage(s, {
    ...
    steps: [
      ...
      ["视觉检测成套引入", "相机 / 光源 / 推理硬件 / 基础判定由专业视觉厂商交付 —— 成熟行业，本方不重复造"],
      ["判定结果接入", "厂商输出的缺陷类型与图像接入平台；不合格联动 MES 拦截，从事后判废转向事中拦截"],
      ["缺陷—参数归因 ★", "把缺陷数据与该批次参数、原料特性关联找出真正相关变量 —— 这是本方的核心增量"],
      ["参数推荐（人机协同）", "数据量支撑后建立推荐模型，输入当批原料特性输出建议参数区间；人工确认后执行"],
    ],
    needs: [
      "缺陷类型与判定标准由质量部门定义，作为视觉厂商的交付要求",
      "原料入厂检测标准化：含水率、粒径不测则模型没有输入变量",
      "视觉厂商输出的缺陷数据须含批次号与时间戳，否则无法与参数对齐（见 P7 规范）",
...
// ═══════════════════════════════ P14 · AI 能力地图 ★
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "AI 能力地图  ·  按研产销分布", "PART 6 · AI CAPABILITY MAP");
  subTitle(s, "全部跑在平台层，现场零部署 · 难度按数据前提排，不按宣传口径排", C.redAccent);

  const rows = [
    ["研", C.rd, "实验记录智能结构化", "研发人员按习惯自由记录，系统自动抽取为结构化字段", "低", "无 · 可立即启用"],
    ["研", C.rd, "工艺知识问答", "新人查询历史相似工况的处置方式，替代口头问老师傅", "中低", "知识库已有内容沉淀"],
    ["研", C.rd, "配方方向推荐", "输入目标性能与成本约束，给出候选配方方向缩小试错范围", "高", "大量配方—性能配对数据"],
    ["产", C.mfg, "缺陷归因分析", "把视觉厂商输出的缺陷数据与工艺参数关联，找出真正相关的变量", "中", "质检结果与参数已配对"],
    ["产", C.mfg, "工艺参数推荐", "按当批原料特性给出建议参数区间，减少调机试错", "高", "长期批次数据积累"],
    ["产", C.mfg, "设备异常预警", "从振动 / 温度 / 电流趋势识别异常征兆，转向计划性维护", "中", "足量故障样本"],
    ["销", C.sales, "认证要求解析", "多市场标准的差异比对、新市场准入要求梳理", "低", "认证文档已入库"],
    ["销", C.sales, "审厂材料组包", "按客户清单从档案库自动生成材料包，替代人工翻找拼凑", "低", "认证档案已结构化"],
    ["销", C.sales, "客诉归因", "投诉关联到批次、工艺参数与配方版本，定位根因", "中", "追溯链已建成"],
    ["跨", C.primary, "园区知识助手", "入驻企业自助查询技术资料、服务申请与流程指引", "中低", "协同平台已上线"],
  ];

  const y0 = 1.72, rh = 0.49;
  s.addShape(p.ShapeType.rect, { x: 0.6, y: y0, w: 12.1, h: 0.32, fill: { color: C.primary } });
  ["", "AI 能力", "解决什么", "难度", "数据前提"].forEach((h, i) => {
    const xs = [0.75, 1.25, 4.05, 9.15, 10.0];
    if (!h) return;
    s.addText(h, {
  ...
  rows.forEach((r, i) => {
    ...
    s.addShape(p.ShapeType.rect, { x: 0.75, y: y + 0.13, w: 0.26, h: 0.23, fill: { color: r[1] } });
    s.addText(r[0], {
      x: 0.75, y: y + 0.135, w: 0.26, h: 0.22,
      fontSize: 7.5, bold: true, color: C.white, fontFace: "Calibri",
      align: "center", margin: 0,
    });
    s.addText(r[2], {
      x: 1.25, y: y + 0.12, w: 2.7, h: 0.26,
      fontSize: 9.5, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
    });
    s.addText(r[3], {
      x: 4.05, y: y + 0.13, w: 5.0, h: 0.24,
      fontSize: 8.2, color: C.gray, fontFace: "Calibri", margin: 0,
    });
    const dc = r[4] === "低" ? C.secondary : r[4] === "高" ? C.redAccent : C.accent;
    s.addShape(p.ShapeType.rect, { x: 9.15, y: y + 0.13, w: 0.62, h: 0.23, fill: { color: dc } });
    s.addText(r[4], {
      x: 9.15, y: y + 0.135, w: 0.62, h: 0.22,
      fontSize: 7.5, bold: true, color: C.white, fontFace: "Calibri",
      align: "center", margin: 0,
    });
    s.addText(r[5], {
      x: 10.0, y: y + 0.13, w: 2.55, h: 0.24,
  ...
  s.addText(
    "注 · 难度按「需要多少数据积累」判定：低 = 处理既有文档即可 · 中 = 需业务数据配对 · 高 = 需长期批次积累并建模。本页不给准确率与提升幅度承诺（见 P15）",
    {
      x: 0.9, y: 6.73, w: 11.5, h: 0.3,
...
// ═══════════════════════════════ P15 · AI 落地路径与边界 ★
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "AI 落地路径与边界", "PART 6 · AI ROADMAP & LIMITS");
  subTitle(s, "先做不依赖产线的文档类，再做需数据配对的分析类，最后做需长期积累的模型类");

  // 左 · 三步走
  s.addText("落地顺序  ·  按数据前提排", {
    x: 0.6, y: 1.72, w: 5.5, h: 0.3,
    fontSize: 12, bold: true, color: C.primary, fontFace: "Cambria", margin: 0,
  });
  const waves = [
    ["第一批  ·  文档类", C.secondary,
      "实验记录智能结构化\n认证要求解析\n审厂材料组包",
      "处理既有文档即可，不依赖产线投产 —— 可与产线建设并行启动"],
    ["第二批  ·  分析类", C.accent,
      "缺陷归因分析\n客诉归因\n工艺知识问答\n园区知识助手",
      "需业务数据配对完成：追溯链建成、质检与参数已关联"],
    ["第三批  ·  模型类", C.redAccent,
      "工艺参数推荐\n配方方向推荐\n设备异常预警",
      "需长期批次与故障样本积累 —— 不承诺短期见效，先做前两批建立信任"],
  ];
  waves.forEach((w, i) => {
    const y = 2.1 + i * 1.62;
    s.addShape(p.ShapeType.roundRect, {
      x: 0.6, y, w: 5.95, h: 1.48,
      fill: { color: C.white }, line: { color: w[1], width: 1.5 }, rectRadius: 0.05,
    });
    s.addShape(p.ShapeType.rect, { x: 0.6, y, w: 5.95, h: 0.34, fill: { color: w[1] } });
    s.addText(w[0], {
      x: 0.82, y: y + 0.04, w: 5.5, h: 0.26,
      fontSize: 10.5, bold: true, color: C.white, fontFace: "Cambria", margin: 0,
    });
    s.addText(w[2], {
      x: 0.82, y: y + 0.42, w: 3.1, h: 1.0,
      fontSize: 8.8, color: C.gray, fontFace: "Calibri",
      lineSpacing: 13, margin: 0, valign: "top",
    });
    s.addShape(p.ShapeType.rect, {
      x: 4.05, y: y + 0.46, w: 0.01, h: 0.9, fill: { color: C.bg2 },
    });
    s.addText(w[3], {
      x: 4.2, y: y + 0.44, w: 2.2, h: 1.0,
      fontSize: 7.8, color: C.accentDk, fontFace: "Calibri",
      lineSpacing: 11, margin: 0, valign: "top",
    });
  });

  // 右 · 明确不做什么
  s.addText("明确不做什么  ·  这比能做什么更重要", {
    x: 6.75, y: 1.72, w: 5.95, h: 0.3,
    fontSize: 12, bold: true, color: C.redAccent, fontFace: "Cambria", margin: 0,
  });
  const nots = [
    ["不做无人干预的自动调参", "模型给建议区间，工艺人员确认后执行；执行结果回写形成闭环。生物基材料工艺窗口窄，自动调参的风险不对称"],
    ["不做产线实时控制", "毫秒级控制与安全联锁由 PLC / DCS 承担 —— AI 在决策层给建议，不进控制回路"],
    ["不做质检的最终裁决", "视觉判定用于拦截与分类，争议批次与边界样本由人工复核；判定标准的解释权在质量部门"],
    ["不承诺无数据支撑的效果", "准确率、提升幅度需在现场样本上验证后才给数字。本方案阶段给能力与前提，不给指标承诺"],
    ["不做黑箱推荐", "参数与配方类推荐须给出依据（相似历史批次、关键变量），工艺人员能判断是否采纳"],
  ];
  nots.forEach((n, i) => {
    const y = 2.1 + i * 0.98;
    s.addShape(p.ShapeType.rect, {
      x: 6.75, y, w: 5.95, h: 0.88,
      fill: { color: "FDF3F2" }, line: { color: C.bg2, width: 0.5 },
    });
    s.addShape(p.ShapeType.rect, { x: 6.75, y, w: 0.05, h: 0.88, fill: { color: C.redAccent } });
    s.addText(n[0], {
      x: 6.98, y: y + 0.1, w: 5.5, h: 0.26,
      fontSize: 10, bold: true, color: C.redAccent, fontFace: "Cambria", margin: 0,
    });
    s.addText(n[1], {
      x: 6.98, y: y + 0.38, w: 5.5, h: 0.46,
  ...
  s.addShape(p.ShapeType.roundRect, {
    x: 0.6, y: 6.98, w: 12.1, h: 0.02,
    fill: { color: C.bg2 }, line: { width: 0 }, rectRadius: 0,
  });
  s.addText(
    "AI 全部部署在平台层，不增加现场部署与运维负担 —— 与 P4 的交付边界一致",
    {
      x: 0.6, y: 7.0, w: 12.1, h: 0.26,
...
// ═══════════════════════════════ P16 · 园区协同平台
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "园区协同平台  ·  把研产销向入驻企业开放", "PART 7 · PARK PLATFORM");
  ...
  const pts = [
    ...
    ["采购结算数据归集", "区内交易的合同与发票数据归集，直接支撑协同采购补贴申报（见 P20）"],
...
// ═══════════════════════════════ P17 · 集成边界
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "系统集成边界  ·  和谁接、接什么", "PART 7 · INTEGRATION");
...
// ═══════════════════════════════ P18 · 建设内容清单
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "建设内容清单  ·  数字化侧", "PART 7 · SCOPE OF WORK");
...
// ═══════════════════════════════ P19 · 技术指标框架
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "技术指标框架", "PART 7 · KPI FRAMEWORK");
...
// ═══════════════════════════════ P20 · 政策对接
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "临港政策对接  ·  按建设内容匹配", "PART 8 · POLICY MAP");
  ...
  ["对应建设内容（P18 编号）", "政策名称", "支持上限", "主要条件"].forEach((h, i) => {
...
// ═══════════════════════════════ P21 · 申报要件对照
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "申报要件对照  ·  谁准备什么", "PART 8 · FILING CHECKLIST");
  subTitle(s, "申报主体为稻生万物 · 下列为通用要件，具体以当年申报指南为准", C.redAccent);

  const rows = [
    ["技术改造和智能化升级", "可研报告 · 投资明细 · 软硬件合同 · 技改前后指标对比",
      "技术方案 · 架构与集成设计 · 建设清单（P18）· 指标框架（P19）", "财务报表 · 固投证明 · 合同与发票"],
...
// ═══════════════════════════════ P22 · 实施路径
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "实施路径  ·  按技术依赖排序", "PART 8 · ROADMAP");
  ...
  s.addText(
    "① 传感器与计量装置必须在产线建设期同步安装 —— 事后加装需停线改造，成本与风险显著上升（见 P7 / P18-02）\n" +
...
// ═══════════════════════════════ P23 · 风险
{
  ...
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "关键风险与应对", "PART 8 · RISK");
...
p.writeFile({ fileName: "daosheng-v20-digital-ai.pptx" })
  .then(() => console.log("✓ daosheng-v20-digital-ai.pptx  ·  24 页"));
