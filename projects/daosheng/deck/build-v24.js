// 稻生万物研产销一体化产业园 · AI Native 数字化方案 · v24 · 27 页
//
// ═══ v24 vs v23 (7/28 鸿波第十二轮) ═══
//
// 鸿波: "从现在条件来看，租赁云主机和云算力更为合理？"
//
// 结论: **当前阶段是的, 而且几乎是唯一合理选择** —— 但这是 PHASE 1 的
// 决策, 不是永久架构决策。新增 P15 把理由、代价与必办事项写清楚。
//
// ═══ 为什么当前条件下云优先 ═══
//
//   ① 需求量算不出来 —— 点位表都没有, 服务器与存储按什么规格买?
//      自建/托管必须现在就按峰值一次性投, 等于闭眼买
//   ② 时间序列错配 —— PHASE 1 的研发数据结构化、认证档案入库**不依赖产线**,
//      今天就该启动; 等机房建好再开工等于白推迟半年以上
//   ③ 没有物理载体 —— 用地未定、厂房未建, 自建机房要等土建
//   ④ 无 7×24 运维能力 —— 制造企业 IT 团队通常不具备(见 P16)
//   ⑤ AI 算力特殊性 —— GPU 采购周期长、贬值快, 冷启动期推理负载极低,
//      买一堆闲置; 模型迭代快, 今年的最优选择明年未必是
//
// ═══ 三个必须摆上台面的代价 ═══
//
//   ① **固投比例** —— 这条我方无依据判断, 必须核实:
//        · 数字化**并入产线技改项目**报 → 产线设备本身是固投主体,
//          云服务费占比小, 影响有限
//        · 数字化**单独立项** → 云订阅几乎零固投, 大概率不达标
//      口径需向管委会确认。它直接决定云优先会不会把申报路堵死。
//   ② **配方数据出厂** —— 缓解手段有(加密/密钥自持/核心配方不进外部模型
//      上下文), 但若客户对外口径含"核心技术自主可控", 就不止是技术问题
//   ③ **长期成本与厂商锁定** —— 规模上来后云总成本可能反超,
//      故现在架构上即须避免绑定专有服务, 保留迁移可能
//
// ═══ 因此 P15 给的是分阶段策略, 不是一次定死 ═══
//   PHASE 1  云优先(需求未知、要快速启动)
//   PHASE 2  用量摸清后评估: 稳定负载可考虑迁本地, 弹性负载留云
//   PHASE 3  按数据分级与申报需要决定最终形态
//
// P16(原 P15 机房选型)定位相应调整为**未来本地化时的选型依据**, 非当前决策。
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - 配置一律 [待核定] · 不编机房参数
//   - **固投口径不臆测** —— 明确标注需向管委会核实
//   - 临港本地算力资源与支持政策未获取, 不臆测
//   - 政策数字标出处页, 不做加总
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 3", "底座  ·  拓扑 · 算力策略 · 机房 · 安全", "P11 - P17", null],
    ["PART 4", "边界  ·  交付分工 · 接入规范 · 治理", "P18 - P20", null],
    ["PART 5", "建设内容 · 指标 · 申报口径", "P21 - P23", null],
    ["PART 6", "政策 · 实施 · 风险 · 下一步", "P24 - P27", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.9 + i * 0.86;
    if (pt[3]) {
...
// ═══════════════════════════════ P12 · 传统系统在 AI Native 里的位置 ★ 消除误解
{
  ...
  const rows = [
    ...
    ["视觉质检系统", "成像与缺陷判定",
      "由专业厂商成套交付", "输出缺陷数据供 Agent 归因（见 P18）"],
    ["报表与看板", "各类统计报表",
...
// ═══════════════════════════════ P14 · 部署形态与算力 ★ 含固投比例提醒
{
  ...
  s.addText(
    "★ 申报关联 · 技改政策要求固定资产投资占比 ≥ 60%（见 P24）。若平台全部采用公有云订阅，该部分计为服务费而非固投，" +
...
// ═══════════════════════════════ P15 · 算力策略 · 当前阶段建议 ★
{
  ...
  const why = [
    ...
    ["时间序列错配", "研发数据结构化、认证档案入库不依赖产线，现在即可启动；等机房建成再开工等于推迟整个 PHASE 1"],
    ["尚无物理载体", "用地未定、厂房未建，自建机房需等待土建；托管亦需选商、签约、上架"],
    ["无 7×24 运维能力", "制造企业 IT 团队通常不具备轮班值守（见 P16 对比）"],
    ["AI 算力的特殊性", "GPU 采购周期长、贬值快；冷启动期推理负载极低，自购易闲置；模型迭代快，选型易过时"],
  ];
  why.forEach((w, i) => {
    const y = 2.02 + i * 0.94;
    s.addShape(p.ShapeType.rect, {
  ...
  const costs = [
    ["★ 固投比例（口径待核实）",
      "云订阅是服务费不是固投。但影响取决于计算口径：\n" +
      "· 并入产线技改项目报 → 产线设备本身是固投主体，云费占比小，影响有限\n" +
      "· 数字化单独立项 → 几乎零固投，大概率不达标\n" +
      "本方案无依据判断，须向管委会确认口径"],
    ["配方数据出厂",
      "缓解：传输与静态加密、密钥自持、核心配方不进入外部模型上下文（P17）、选择合规服务区。\n" +
      "但若对外口径含「核心技术自主可控」，则不止是技术问题"],
    ["长期成本与厂商锁定",
      "规模上来后云总成本可能反超自建。架构上须避免绑定专有服务，保持数据与模型可导出，预留迁移路径"],
  ];
  costs.forEach((c, i) => {
    const h = i === 0 ? 1.62 : 1.12;
    const y = 2.02 + (i === 0 ? 0 : (i === 1 ? 1.72 : 2.94));
    s.addShape(p.ShapeType.roundRect, {
      x: 6.75, y, w: 5.95, h,
      fill: { color: "FDF3F2" },
      line: { color: C.redAccent, width: i === 0 ? 1.8 : 1 }, rectRadius: 0.05,
    });
    s.addText(c[0], {
  ...
  s.addText(
    "PHASE 1 云优先（需求未知、快速启动）  →  PHASE 2 用量摸清后评估：稳定负载可迁本地、弹性负载留云  →  PHASE 3 按数据分级与申报需要定最终形态",
    {
  ...
  s.addText(
    "① 向管委会核实固投计算口径   ② 完成数据分级：定清哪些绝对不能出厂区   ③ 架构避免厂商锁定，保留迁移路径   ④ 核实临港本地算力资源与支持政策",
    {
...
// ═══════════════════════════════ P16 · 算力中心 · 自有机房 vs 租赁托管 ★
{
  nextP();
  const s = p.addSlide();
  s.background = { color: C.bg };
  pageTitle(s, "算力中心  ·  自有机房 vs 租赁托管", "PART 3 · DATA CENTER SITING");
  subTitle(s, "当前阶段建议云优先（见 P15）· 本页为未来负载稳定后若转本地化的选型依据", C.redAccent);
...
// ═══════════════════════════════ P17 · 安全与合规体系 ★
{
  ...
  const layers = [
    ["物理与网络", C.steel,
      "机房物理安防（自建 or 托管的选型见 P16）· OT / IT 隔离（工业防火墙或网闸）\n区域划分与最小开放 · 入侵检测 · 外部接入边界防护与审计"],
    ["数据安全", C.rd,
...
// ═══════════════════════════════ P21 · 建设内容清单
{
  ...
  const rows = [
    ...
    ["05", "算力与存储", "PHASE 1 云订阅；后续如转自购则为服务器与推理算力", "选型本方", "云=服务费/自购=固投"],
    ["05b", "机房或托管（后续）", "转本地化时：自建土建配套 或 托管机柜与专线", "建设方", "技改（固投/费用）"],
  ...
  s.addText(
    "注 · 05-06 为固定资产投资，直接关系技改申报的固投比例（见 P14）· 08-11 是 AI Native 特有内容，申报口径说明见 P23",
    {
...
// ═══════════════════════════════ P24 · 政策对接
{
  ...
  const rows = [
    ...
    ["05 06  算力与安全", "企业技术改造（固定资产投资）", "自购计固投 / 云订阅计服务费", "★ 口径待核实，见 P15"],
  ...
  ["对应建设内容（P21 编号）", "政策名称", "支持上限", "主要条件"].forEach((h, i) => {
  ...
  s.addText(
    "逐条摘自《上海临港新片区政策情况》p7-p12 · 以临港新片区管委会发布最新版本为准 · 申报主体为稻生万物 · 申报表述见 P23",
    {
...
// ═══════════════════════════════ P25 · 实施路径
{
  ...
  s.addText(
    "① 传感器与计量装置必须在产线建设期同步安装 —— 事后加装需停线改造（见 P19 / P21-02）\n" +
...
// ═══════════════════════════════ P26 · 风险
{
  ...
  const risks = [
    ...
    ["集成商交付不达标", "产", "数据接进来但缺点位、时间戳不同步，上层无法使用",
      "P19 验收标准作为付款节点前置条件 · 联调阶段本方参与"],
    ["期望管理失控", "AI", "「AI 什么都能干」的预期与实际能力落差，导致信任崩塌",
      "P20 明确不做什么 · 不承诺无数据支撑的效果 · 分期兑现建立信任"],
    ["开放边界失守", "销", "技术与客户资源开放给入驻企业后被绕开自立门户",
      "权限分级在系统中强制执行，但边界须先在入驻协议中逐项约定"],
    ["基线缺失无法验收", "—", "指标基线未实测就申报，验收时拿不出改造前数据",
      "PHASE 1 首要任务即基线测评 · 采集方法见 P22"],
    ["固投比例不达标", "—", "PHASE 1 采用云订阅，若数字化单独立项则固投占比可能不足",
      "先向管委会核实计算口径（P15）· 若不达标则调整为部分自购，或并入产线技改项目一并申报"],
  ];
  risks.forEach((r, i) => {
    const y = 1.7 + i * 0.66;
    s.addShape(p.ShapeType.rect, {
...
p.writeFile({ fileName: "daosheng-v24-ai-native-cloud.pptx" })
  .then(() => console.log("✓ daosheng-v24-ai-native-cloud.pptx  ·  27 页"));
