// 稻生万物研产销一体化产业园 · AI Native 数字化方案 · v22 · 25 页
//
// ═══ v22 vs v21 (7/28 鸿波第十轮) ═══
//
// 鸿波: "是不是要画出实现拓扑，包括算力、安全设施等等？"
//
// 要画。而且有个理由比技术层面更紧迫:
//
//   **技改政策要求固投比例 ≥ 60%**（见政策页条件列）。
//   若方案全是软件与云服务, 那是服务费不是固定资产投资 ——
//   固投比例大概率不达标, 申报直接卡住。
//   所以算力与安全硬件不只是技术问题, 是**申报可行性问题**。
//
// 另两个理由:
//   ② 等保（网络安全等级保护）对制造企业是**准入门槛不是加分项**,
//      涉及工业控制系统通常需二级或三级, 评审必问
//   ③ AI 的算力构成与传统 IT 不同（推理服务 / 向量检索 / 模型托管）,
//      不写清楚客户不知道要准备什么
//
// ═══ 新增三页 ═══
//   P13 部署拓扑        四区划分 + 网络边界 · 明确"车间零部署"不等于"没有机房"
//   P14 部署形态与算力   三方案对比 + 算力分类核定依据 + ★固投比例提醒
//   P15 安全与合规体系   四层防护 + **AI 特有安全** + 等保定级
//
// ═══ 一个必须讲清的区分 ═══
//
// v20-v21 讲的"现场零部署"指的是**车间现场（OT 侧）零部署** ——
// 产线边缘的网关、采集、视觉推理硬件均由集成商交付。
// 但平台层仍需算力承载, 部署在**厂区机房或云（IT 侧）**, 不进车间。
// 两者不矛盾, 但 P13 必须画清楚, 否则读者会以为方案不需要任何基础设施投入。
//
// ═══ 配置一律不编 ═══
//   服务器型号、GPU 数量、存储容量取决于点位数、采样频率、图像量与用户数,
//   本方案阶段无法给出。P14 只给**分类与核定依据**, 具体数值标 [待核定]。
//   编一个数字的后果: 概算错、采购错、申报材料与实际不符。
//
// ═══ 延续的结构性决定 ═══
//   ① AI Native 骨架: 交互 → Agent 编排 → 能力知识 → 数据语义 → 接入
//   ② 四个 Agent: 研发 / 生产 / 合规客户 / 园区
//   ③ 交付边界在数据接入层, 设备与视觉硬件交集成商, 接口规范本方定义
//   ④ 传统系统仍建, 但降级为 Agent 可调用的工具与数据源
//
// ═══ 军规 ═══
//   - 不点"鲶鱼"品牌名 · 不出现"自主研发/自研"
//   - 不写人月工期
//   - 指标基线 [待实测] / 目标 [待定] / **算力配置 [待核定]**
//   - AI 效果不承诺数字
//   - 政策数字标出处页, 不做加总
//
// 数据来源:
...
// ═══════════════════════════════ P3 · 目录
{
  ...
  const parts = [
    ...
    ["PART 3", "底座  ·  数据知识 · 拓扑 · 算力 · 安全", "P11 - P15", null],
    ["PART 4", "边界  ·  交付分工 · 接入规范 · 治理", "P16 - P18", null],
    ["PART 5", "建设内容 · 指标 · 申报口径", "P19 - P21", null],
    ["PART 6", "政策 · 实施 · 风险 · 下一步", "P22 - P25", null],
  ];
  parts.forEach((pt, i) => {
    const y = 1.9 + i * 0.86;
    if (pt[3]) {
...
// ═══════════════════════════════ P12 · 传统系统在 AI Native 里的位置 ★ 消除误解
{
  ...
  const rows = [
    ...
    ["视觉质检系统", "成像与缺陷判定",
      "由专业厂商成套交付", "输出缺陷数据供 Agent 归因（见 P16）"],
    ["报表与看板", "各类统计报表",
...
// ═══════════════════════════════ P14 · 部署形态与算力 ★ 含固投比例提醒
{
  ...
  s.addText(
    "★ 申报关联 · 技改政策要求固定资产投资占比 ≥ 60%（见 P22）。若平台全部采用公有云订阅，该部分计为服务费而非固投，" +
...
// ═══════════════════════════════ P19 · 建设内容清单
{
  ...
  s.addText(
    "注 · 05-06 为固定资产投资，直接关系技改申报的固投比例（见 P14）· 08-11 是 AI Native 特有内容，申报口径说明见 P21",
    {
...
// ═══════════════════════════════ P22 · 政策对接
{
  ...
  ["对应建设内容（P19 编号）", "政策名称", "支持上限", "主要条件"].forEach((h, i) => {
  ...
  s.addText(
    "逐条摘自《上海临港新片区政策情况》p7-p12 · 以临港新片区管委会发布最新版本为准 · 申报主体为稻生万物 · 申报表述见 P21",
    {
...
// ═══════════════════════════════ P23 · 实施路径
{
  ...
  s.addText(
    "① 传感器与计量装置必须在产线建设期同步安装 —— 事后加装需停线改造（见 P17 / P19-02）\n" +
...
p.writeFile({ fileName: "daosheng-v22-ai-native-topo.pptx" })
  .then(() => console.log("✓ daosheng-v22-ai-native-topo.pptx  ·  25 页"));
