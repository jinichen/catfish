...
- **vendored 第三方代码** (8/15 加): 任何**祖先目录里有 LICENSE 文件**的源文件
  (仓根那份 Apache 2.0 不算)。

  理由: 那些目录整个是抄进来的, 不是我们写的。就地拆有两个问题 —— 下次同步
  上游改动被冲掉; 改完跟上游 diff 不上, 想跟进对方的 bugfix 只能手动挑。
  要动只能 fork 或者提 PR 给上游。

  判据是"**真有一个 LICENSE 文件躺在那**", 不是看目录叫不叫 vendor —— 跟
  schema 那条"不能光看文件名"同理, 否则豁免就成了藏污纳垢的地方。

  ⚠ 同样**单独列出来**, 超线的还会点名。万一哪天有人往自己的目录里放了个
  LICENSE, 半个仓会悄悄从报告里消失, 这个数字是唯一能看出来的地方。
- **点名清单** (8/15 加): `scripts/check_file_sizes.sh` 里的 `NAMED_EXEMPT_LIST`。

  上面几条都是**类别**规则 (判据能自动判), 这一条是"人看过之后拍板留着"的,
  所以只能一个一个点名, 而且每一项旁边必须写理由。

  有意**不**做成类别规则 —— "交付脚本"、"文案生成器" 这种类别一旦开口, 下一个
