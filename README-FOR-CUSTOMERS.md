...
> 央企 / 大型组织的 **员工成长加速器 + 组织能力沉淀器** — 100% 私有部署, 数据不出客户内网.
> 1 页快速了解, 5 分钟知道是不是适合你们.

## 一句话

鲶鱼让员工**个人能力持续成长**, 把员工愿意分享的部分**沉淀成企业资产**. 数据全在员工电脑, 跳槽带走; 员工显式贡献的 skill 才进部门 Hub. **双层飞轮**, 跟人才战略 + 数字化转型对齐.

## 双层价值 (5/21 加): 员工的 + 企业的, 边界清晰

> 鲶鱼**不是**给公司装的监控工具, **也不是**装在云端的通用 SaaS. 是**两层资产并存**:

| 层 | 员工的 (跟员工走) | 企业的 (沉淀部门) |
|---|---|---|
| **数据** | `~/.catfish/` 全部 (memory / journal / chat / 风格指纹 / 私聊) — 跳槽 cp 走 | 中央 PG metadata (谁调用 / 几次 / 哪个模型, **没对话内容**) |
| **Skills** | 教学产物默认本机 `~/.catfish/skills/` (员工教鲶鱼的私人流程) | 员工**显式 publish** 才入部门 Skills Hub (员工自愿贡献, 不是被抽数据) |
| **价值** | 员工**个人能力成长** — 6 个月鲶鱼跟你磨合, 越用越懂你 | **部门 know-how 数字化沉淀** — 50 人部门 1 年长 50-100 个工作流模板 |
| **跳槽时** | 全部带走, 6 个月积累不丢 | 已 publish 的 skill 留在部门, 老员工流失, 流程不流失 |

**关键**: 企业能力提升**来自员工自愿贡献** (显式按钮 + 3 道安全扫描 + manager 审核), **不是从员工电脑抽数据**.
