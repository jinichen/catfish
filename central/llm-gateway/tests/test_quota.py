...
# ─── friendly_quota_message (P3.5.29 + P3.5.139; 9/23 改推导) ─────────────
#
# friendly_message 里"建议换的 model 名" 由 _resolve_friendly_model 拿, chain:
#   1. roles.resolve_or_none("chat_default") —— 9/23 起从模型页「默认」徽章算出来
#   2. .env CATFISH_FALLBACK_CHAT_DEFAULT —— 一个对话模型都没有时兜底
#   3. 空字符串 (文案降级)
#
# 9/23 之前还有 public_flash ("换 X 或 Y") —— 那个角色在代码里唯一的用处就是
# 这句文案里的一个词, 跟着 roles.yaml 一起删了。


@pytest.fixture
def chat_default_stub(monkeypatch: pytest.MonkeyPatch):
    """把 roles.resolve_or_none 打桩成可控值 (None = 推不出)."""
    from catfish_gateway import roles as roles_module

    def _set(value: str | None) -> None:
        monkeypatch.setattr(
            roles_module, "resolve_or_none",
            lambda role: value if str(getattr(role, "value", role)) == "chat_default" else None,
        )

    return _set


def test_friendly_message_per_user_minute(chat_default_stub) -> None:
    """推得出默认对话模型 → 文案带它的名字."""
    chat_default_stub("test-main-model")
    qc = quota.QuotaCheck(
        allowed=False, dimension="per_user_minute",
        current=105000, limit=100000, reset_at=int(time.time()) + 30,
    )
...
def test_friendly_message_per_model_day(chat_default_stub) -> None:
    chat_default_stub("test-main-model")
    qc = quota.QuotaCheck(
        allowed=False, dimension="per_model_day",
        current=5000000, limit=5000000, reset_at=int(time.time()) + 86400,
    )
...
def test_friendly_message_env_fallback(chat_default_stub, monkeypatch: pytest.MonkeyPatch) -> None:
    """推不出 (一个对话模型都没有) → 走 .env CATFISH_FALLBACK_CHAT_DEFAULT."""
    chat_default_stub(None)
...
def test_friendly_message_no_env_returns_empty(chat_default_stub, monkeypatch: pytest.MonkeyPatch) -> None:
    """推不出 + .env 没配 → 文案降级渲染空字符串.

    略丑 ("换 (内网不限)"), 但比静默走硬编码字面值清晰. dev 启动 gateway
    没模型时可见, 不该出现在正常 production.
    """
    chat_default_stub(None)
