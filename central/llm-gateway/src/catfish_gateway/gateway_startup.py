...
def _seed_and_migrate_models() -> None:
    ...
    try:
        ...
        if fixed:
            ...
            invalidate_config()
        # 同一个 mode 里撞出多个 default 时清到只剩一个 (对话一个、向量一个是
        # 正常状态, 9/23 起按 mode 分)。default_model() 返回第一个 default=True
        # 的, 两个的话就取决于排序, 员工下次开聊用哪个模型不可预测。
...
async def run_startup(app, env_file_loaded):
    ...
    # 9/23: 没有 roles.yaml 了。角色 (/v1/roles) 从模型页的「默认」徽章算出来,
    # 见 roles.py 开头。这里只剩一件事要在启动时**说出来**: 向量模型多个但一个
    # 都没勾默认 —— /v1/embeddings 不带 model 会 400, 而 Companion 拿到非 200
    # 会静默退回本机 ONNX (Windows 没编 ort, 等于没有向量)。
    try:
        _cfg = get_config()
        _emb = [m.name for m in _cfg.models if m.mode == "embedding"]
        if len(_emb) > 1 and _cfg.default_embedding_model() is None:
            logger.error(
                "⚠️ 有 %d 个向量模型 (%s) 但一个都没勾「默认」—— /v1/embeddings 不带 "
                "model 的请求会 400, Companion 会静默退回本机向量。去控制台 → 模型, "
                "给一个向量模型勾上「默认」。",
                len(_emb), ", ".join(_emb),
            )
        if not _emb:
            logger.warning(
                "没有向量模型 —— 语义搜索 / advisor 相关度 / 记忆检索走员工本机向量; "
                "Windows 客户端 (msi 没编 ort) 完全没有向量。不用中央向量是合法选择, "
                "但要知道这一点。"
            )
        if _cfg.default_model() is None:
            logger.error("⚠️ 一个对话模型都没有 —— hermes 的 catfish-auto 会 500, 选择器没有初始值。")
    except Exception:  # noqa: BLE001
        logger.exception("默认模型自检本身出错 (不影响启动)")
