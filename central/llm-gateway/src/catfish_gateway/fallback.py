...
_ERROR_KEYWORDS = {
    ...
    #
    # ⚠ 8/9: 这组关键词**当前一个模型都没配** —— 是有意的, 不是漏了。
    #
    # 8/8 我顺手给 4 个公网模型加进了 on_errors, 鸿波 8/9 让撤回。理由成立:
    # on_errors 是严格 opt-in 白名单 (should_fallback 匹配不上就 return False),
    # 而这条 opt-in 语义是保密性的支点 —— 内网模型故意不配 500, 靠的就是
    # "不列就永不切"。往这张表里加东西要按同一个标准慎重。
    # 且余额不足是**账务状态**不是瞬时故障: 自动切换会把问题藏起来、把开销
    # 悄悄挪到别家、可能违反部门"就用这家"的约定。开不开是产品决策。
    #
    # 留着关键词组的意义: 哪天决定要开, 在管理控制台给具体模型加一条就行,
    # 代码不用动。tests/test_fallback_500_policy.py 里
    # test_当前_yaml_里公网模型确实没配_是有意的 钉住了这个状态。
    #
