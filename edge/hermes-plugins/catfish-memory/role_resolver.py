...
# _get_summarize_model 优先级 chain:
...
httpx 没装 / gateway 挂 / JSON 错 → 返 None. caller 走自己
fallback (yaml/env). 不阻塞 hermes plugin sync_turn.
"""
...
def _gateway_url() -> str:
    """env var pattern 跟 tool-bridge / Companion 对齐."""
...
def resolve(role: str) -> Optional[str]:
    """核心 API: role name → 物理 model name. 失败返 None.

    Args:
        role: 真 ``roles.yaml`` role 名 (e.g. ``"summarize"``)

    Returns:
        物理 model name 或 None.
    """
