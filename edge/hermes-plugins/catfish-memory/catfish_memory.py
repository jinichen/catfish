...
class CatfishMemoryProvider(MemoryProvider):
    ...
    def _render_fact_first_discipline(self) -> str:
        ...
        # 老措辞用"自评报告/运行记录/访谈提纲"作反面禁止例子, LLM negation
        # blindness 反而复现 (常见 bug '不要想大象'→都在想大象). 抽象化措辞,
        # 移除所有具体名词锚点, 只描述**行为原则**, 让 LLM 无具体词可复现.
        return (
            ...
            "- 从行业模板 / 通识 / 上下文推理凭空**编具体名词**给员工. "
            "员工没提的材料名 / 步骤名 / 系统名 / 日期都不要说. "
            "想推荐一个通用做法 → 让员工确认, 不要直接列具体清单.\n"
            ...
            "**冲突时优先级**: 员工 chat 原话 > 早晨 briefing 元数据 > memory prefetch 注入内容.\n\n"
            "**核心哲学**: 你是**参谋**不是**作文**. 员工要事实 + 授权动作, "
            "不要 AI 生动化 + 假设推理.\n"
        )
