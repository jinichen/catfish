"""BL-LEARN-RECMODE / CDP listener (5/14 v1, 5/26 搬到 edge/tool-bridge).

# 5/26 BL-RECMODE-MIGRATE-TO-EDGE batch 1 (E.2): 从 central/llm-gateway/.../recmode/
# 搬这里. 跟 5/25 搬 aggregator + selector_repair 同模式. gateway /api/learn/*
# 5 个 endpoint (start/stop/active/status/cleanup) 改 thin proxy 通过 Unix socket
# JSON-RPC 转过来. central 端剩 fail-loud stub (RuntimeError on import).
#
# 真实搬迁原因 (鸿波 5/26 反思): 边界 doc E 类清零 — central/ 代码不再读写
# ~/.catfish/recordings/. SaaS 化时 gateway 独立部署不会破录屏承诺.
...
跑法:
    from catfish_tool_bridge.recmode.cdp_listener import CDPRecordingSession
