...
# 9/18: Foxmail 老式 .box 夹具连同 box_parser 一起删了。
#
# 当初这里留过一句话, 今天字字应验:
#   "这些测试证明的是 parser 逻辑符合我们记录的格式, **不**证明能读真的
#    Foxmail 文件 —— 仓库里没有任何一个真 .box 样本。第一件事应该是弄一个
#    真样本来对, 而不是相信这里的绿。"
#
# 没照做。28 条测试绿了四个月, 到真机上一次爆出六个 bug, 最后一个是
# Foxmail 7.2 把邮件文件加密了 —— 解析器写得再对也读不出东西。
# 现在改成读客户端导出的 .eml (RFC822 标准), 夹具用真实格式造, 见 eml_dir。
