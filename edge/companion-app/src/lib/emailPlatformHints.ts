...
export function emailFailureHint(platform = currentEmailPlatform()): string {
  if (platform === "windows") {
    return "常见原因：Foxmail 尚未配置账号、Foxmail 尚未完成同步，或邮件组件尚未完成安装。若 Foxmail 数据目录不在默认位置，请在 %USERPROFILE%\\.catfish\\companion.yaml 的 email.foxmail_root 配置真实 Storage 目录；若仍失败，日志位于 %LOCALAPPDATA%\\hermes\\logs\\catfish-companion-bootstrap.log。";
  }
  if (platform === "macos") {
