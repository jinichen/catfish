...
export default function EmailTab() {
  ...
  const [actionFilter, setActionFilter] = useState<ActionFilter>("全部");  // 8/21 分诊筛选
  ...
  // 8/15 永动机 (83 分钟烧 2470 万 token) 的修复护栏和长注释都在那边,
  // 改扫描逻辑先去读它。
  useEmailScanners({
    items,
    urgencyMap,
  ...
  // 8/21: 搜索 + 分诊筛选/排序合并进纯函数 (EmailTab 顶着 800 行红线;
  // 排序判据「办里无截止日的排哪」值得单测, UI 里肉眼看不出错)。
  const filteredItems = useMemo(
    () => filterAndRankEmails(items, { search, actionFilter, actionMap }),
    [items, search, actionFilter, actionMap],
  );
  const actionCounts = useMemo(() => {
    const c: Record<string, number> = {};
    for (const m of items) {
      const a = actionMap[m.id]?.action;
      if (a === "办" || a === "回") c[a] = (c[a] || 0) + 1;
    }
    return c;
  }, [items, actionMap]);
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", fontSize: 11, marginBottom: 6 }}>
            ...
            {/* 8/21 分诊筛选: 全部/办/回。放同一行 (340px 左栏是临界宽度) */}
            <ActionFilterChips
              value={actionFilter}
              counts={actionCounts}
