...
早期 Wiki 可能使用 `related: ["[[目标条目]]"]`，这种格式只有目标没有关系类型。
关系整理页会把它标为“关系异常”，但这不代表目标一定断链。点击“检查旧关系”后，
可以一次性把这类关系规范化为 `rel: "关联"`；原目标和正文不变，执行前会备份到
`~/.catfish/wiki/.migration-backups/`。找不到目标或存在歧义的关系不会被自动猜测，
仍需人工确认。
