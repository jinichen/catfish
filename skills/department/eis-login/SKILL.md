...
deprecated: false
description: |-
  ⭐ 登录 EIS 一站式信息门户 (含动态验证码识别 + 跳转校验)

  ⚙️ 由 catfish_freeze_skill 自动凝固 (2026-05-12 10:39:42, BL-MM9-FREEZE).
  源 trace: /Users/chenhongbo/.catfish/traces/active.jsonl (steps 1-7, ok=7/7).
  **不要手改 script.py** — 业务流程变了走"再教一次"路径让管道重新凝固.

  调用入口: `render_eis_login(username, password_ref, max_captcha_retry)` (script.py).
---

# eis-login — 登录 EIS 一站式信息门户 (含动态验证码识别 + 跳转校验)

> 凝固于 2026-05-12 10:39:42. 教学→凝固→复用闭环 (BL-MM9-FREEZE).
