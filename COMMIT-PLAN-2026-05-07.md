# Commit 拆分计划 · 5/4-5/7 累计 (66 文件)

> 自上次 commit `aab5eb9 桌宠独立形象` 后, 5/4-5/7 4 天累计 66 文件改动.
> 按 sprint 阶段拆 6 个 commit, 每个 commit 能独立 ship 且通过测试.
...
cd ../companion-app
npx tsx src/lib/triggers.test.ts  # 16 trigger tests

cd ../../central/llm-gateway
python3 -m pytest tests/test_proactive_contextual.py -v  # 8 tests
```

---

## Commit 1 · BL-MM7 + BL-MM8 (用户画像 + 风格指纹后端)
...
git commit -m "BL-MM7+MM8: 用户画像 + 风格指纹完整链路 ship
...
git commit -m "Phase 2.5 真主动: 信号触发 + LLM context 化 + 桌宠 polling fix
...
git add .github/
git add .security/
git commit -m "G1-G7 安全 7 GAP 一气呵成 ship
...
## Commit 4 · BL-S29 真技术沙箱 (12 天压 1 天 ship)

```bash
git add edge/tool-bridge/sandbox-profiles/
git add edge/tool-bridge/src/catfish_tool_bridge/sandbox.py
git add edge/tool-bridge/src/catfish_tool_bridge/adapter.py  # 沙箱 dispatch 拦截分支
...
git commit -m "BL-S29 真技术沙箱 12 天 sprint 单日 ship — 86 测试全绿
...
- 加 obfuscation (base64/eval/chr) / 跨进程 (ps/dscl/osascript) / 内核级 (dlopen/setns/kexec)
...
git commit -m "客户文档矩阵 + Plan D 重定位 (员工职业资产 / 跨雇主可携带)
...
- QUICKSTART-EMPLOYEE
- DEMO-EMERGENCY-PLAN / RUNBOOK-DEMO-VERIFICATION
...
## Commit 6 · 杂项 fix + CHANGELOG 5/6+5/7
...
git commit -m "杂项 fix + CHANGELOG 5/6+5/7
...
```bash
git add -A
git commit -m "5/4-5/7 累计 sprint: BL-MM7+MM8 + 真主动 + G1-G7 + BL-S29 + 客户文档"
```

但**强烈推荐拆 6 commit**, 软著申报 / 跟同事 review 时按 sprint 看清晰.
